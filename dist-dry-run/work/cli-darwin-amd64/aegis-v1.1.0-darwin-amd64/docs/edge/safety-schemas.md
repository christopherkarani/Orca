...
- `schemas/edge-event-v1.json`: event name surface for Edge audit events.
- `schemas/safety-report-v1.json`: safety report shape with findings, limitations, and non-certification disclaimer fields.
...
Phase 31 prepares Core audit events for local policy decisions, fake MAVLink gateway decisions, fake-PX4/fake-ArduPilot scenarios, opt-in PX4/ArduPilot SITL simulation evidence, and safety findings using these names. It does not emit real-flight events.

## Safety Report V1

Safety reports support report id, version, vehicle profile, adapter profile, policy hash, scenario name/source, test environment, checks run, commands allowed/denied, violations, structured findings, audit event references, limitations, and a non-certification disclaimer.

Valid test environment labels are fake adapter, PX4 SITL, ArduPilot SITL, bench, and other. PX4 SITL and ArduPilot SITL are opt-in local simulation labels. These labels are not claims of real flight.
