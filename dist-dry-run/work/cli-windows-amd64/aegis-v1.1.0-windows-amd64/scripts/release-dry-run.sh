...
DIST_DIR="${AEGIS_DIST_DIR:-dist-dry-run}"

# Verifies release archive checksums through scripts/verify-release.sh.
printf 'release dry-run: building artifacts into %s\n' "$DIST_DIR"
AEGIS_DIST_DIR="$DIST_DIR" ./scripts/build-release.sh
./scripts/verify-release.sh "$DIST_DIR"

tmp_dir="$(mktemp -d "${TMPDIR:-/tmp}/aegis-release-dry-run.XXXXXX")"
cleanup() {
  rm -rf "$tmp_dir"
}
trap cleanup EXIT INT TERM

edge_artifact="$(ls "$DIST_DIR"/aegis-edge-v*-linux-amd64.tar.gz 2>/dev/null | head -n 1 || true)"
if [ -n "$edge_artifact" ]; then
  tar -xzf "$edge_artifact" -C "$tmp_dir"
  edge_bin="$(find "$tmp_dir" -type f -path '*/bin/aegis-edge' -perm -111 | head -n 1)"
