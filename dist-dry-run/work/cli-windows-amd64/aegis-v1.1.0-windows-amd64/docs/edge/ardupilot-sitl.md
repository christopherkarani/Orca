...
```bash
AEGIS_EDGE_RUN_ARDUPILOT_SITL_TESTS=1 AEGIS_EDGE_ARDUPILOT_ENDPOINT=127.0.0.1:14550 AEGIS_EDGE_ARDUPILOT_VEHICLE=copter zig build test
```

The tested ArduPilot version is recorded as `documented-by-phase-30` unless `AEGIS_EDGE_ARDUPILOT_TESTED_VERSION` is set for a local run. Normal tests do not require ArduPilot.
