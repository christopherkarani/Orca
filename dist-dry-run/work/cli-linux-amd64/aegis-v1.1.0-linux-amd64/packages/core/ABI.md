...
- `aegis_core_version`
- `aegis_core_redact`
- `aegis_core_evaluate_policy`
- `aegis_core_append_audit_event`
...
Callers own all input and output buffers. Aegis Core does not allocate memory for the caller and never frees caller memory.
...
String inputs must be UTF-8 where the underlying Core API requires UTF-8. Event-field-sized inputs must not exceed Aegis Core runtime limits.
