...
Phase 31 adds a reusable Aegis Edge safety layer for fake-adapter, PX4 SITL, and ArduPilot SITL contexts. The public API is `edge.safety.evaluateSafety(policy, vehicle_state, command_request, context)` and mission uploads use `edge.safety.evaluateMissionSafety(...)`.
...
Aegis Edge is not a flight controller, not an autopilot replacement, not detect-and-avoid, not regulatory approval, and not real-flight readiness.
