...
Polished the Aegis repository for public release by rewriting the root `README.md`, updating `.gitignore` to exclude planning artifacts, and removing tracked planning/scratch files from the git index without deleting local files.

## README Changes

Rewrote `README.md` to be concise, launch-ready, and honest. Key changes:

- Added **Why Aegis Exists** section explaining the problem Aegis solves.
- Added **What Aegis Does** section with bullet points for `aegis run`, policy checks, secret redaction, audit logs, replay, red-team fixtures, plugin doctor, Codex plugin, and Claude Code plugin.
...
- Expanded **What Aegis Does Not Promise** with the required limitation bullets, including:
  - "The current plugin release does not add MCP server behavior or drone-specific plugin features."
- Removed the extensive Aegis Edge/drone phase history and operational details from the root README (those remain in `packages/edge/README.md` and dedicated docs).
- Added the required exact wording:
  - "The strongest local protection remains running the agent through `aegis run`; plugins provide native commands, hooks, and guardrails inside supported agent hosts."
...
- Aegis internal planning / generated plan packs (directories and zips)
- Local planning scratch (`planning/`, `plans/`, `.plan/`, `.local-plans/`, `scratch/`, `tmp/`, `*.tmp`, `*.bak`)
- Local agent/planning exports (root-level planning markdown files like `CODEX_MASTER_PROMPT.md`, `P00_*.md`, `[0-9][0-9]_*.md`, `AEGIS_CLI_PLUGIN_CONTRACT.md`, `ARCHITECTURE_CONTRACTS.md`, `CODEX_AGENT_CONTEXT.md`, `DRONE_WORKSTREAM_GUARDRAILS.md`, `FINAL_PASS_REPORT.md`, `REVIEW_SUMMARY.md`, `SECURITY_INVARIANTS.md`, etc.)
...
- `P01_AEGIS_CLI_PLUGIN_SURFACE.md`
...
| `./zig-out/bin/aegis doctor` | Passed |
| `./zig-out/bin/aegis redteam --ci` | Passed (10/10 fixtures) |
| `./zig-out/bin/aegis plugin doctor codex` | Passed |
| `./zig-out/bin/aegis plugin doctor claude` | Passed |
| `./zig-out/bin/aegis plugin manifest codex` | Passed |
| `./zig-out/bin/aegis plugin manifest claude` | Passed |
...
- **Scope**: The README does not claim unsupported features. Phrases like "perfect sandbox", "MCP server", and "drone-specific" appear only in the "What Aegis Does Not Promise" limitations section.
