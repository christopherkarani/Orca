...
Edge Phase 41 supports source, packaged, container, simulation, and no-actuation bench-preparation deployment checks.
...
```bash
edge deployment doctor
edge deployment assets
edge deployment check --profile examples/edge/deployment/profiles/source-local-fake.yaml
```
