...
The Phase 27 Edge policy engine lives in `packages/edge/src/policy/`. It loads Edge policy version `1`, validates the Phase 26 domain model, and evaluates `CommandRequest` plus `VehicleState` into the shared Aegis Core decision model.
...
`aegis-edge policy check` validates a policy. `policy explain` evaluates one command using deterministic fake state. `policy evaluate` evaluates a JSON request and JSON state without forwarding anything.
