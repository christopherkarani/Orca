...
Edge evaluates commands against policy, safety envelope constraints, operator approval rules, emergency policy, telemetry/data guard rules, and runtime health.

## Architecture

Agent or companion planner requests flow through Edge before fake adapters, MAVLink gateway paths, or opt-in SITL contexts. Edge does not replace the autopilot.
