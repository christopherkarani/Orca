...
const MAV_MODE_FLAG_AUTO_ENABLED: u8 = 0x04;
const MAV_MODE_FLAG_GUIDED_ENABLED: u8 = 0x08;
const MAV_MODE_FLAG_STABILIZE_ENABLED: u8 = 0x10;
const MAV_MODE_FLAG_MANUAL_INPUT_ENABLED: u8 = 0x40;
...
pub const StateMapper = struct {
    ...
    fn observeHeartbeat(self: *StateMapper, frame: mavlink.framing.Frame) !void {
        ...
        self.current.autopilot_kind = if (heartbeat.autopilot == MAV_AUTOPILOT_PX4) .px4 else .unknown;
        self.current.arm_state = if ((heartbeat.base_mode & MAV_MODE_FLAG_SAFETY_ARMED) != 0) .armed else .disarmed;
        self.current.mode = modeFromBaseMode(heartbeat.base_mode);
...
fn modeFromBaseMode(base_mode: u8) domain.vehicle.VehicleMode {
    if ((base_mode & MAV_MODE_FLAG_MANUAL_INPUT_ENABLED) != 0) return .manual;
    if ((base_mode & MAV_MODE_FLAG_AUTO_ENABLED) != 0) return .auto;
    if ((base_mode & MAV_MODE_FLAG_GUIDED_ENABLED) != 0) return .guided;
    if ((base_mode & MAV_MODE_FLAG_STABILIZE_ENABLED) != 0) return .stabilized;
    return .guided;
}
