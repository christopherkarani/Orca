#!/bin/bash
#
# End-to-End Test Script for orca
#
...
#   --binary      Path to orca binary (default: searches Cargo target dirs, then PATH)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --binary, -b      Path to orca binary (default: Cargo target dirs, then PATH)"
...
# Find binary
if [[ -z "$BINARY" ]]; then
    # Prefer in-repo build artifacts over any globally installed `orca` to keep
    ...
    if [[ -n "${CARGO_TARGET_DIR:-}" ]]; then
        ...
        BINARY_CANDIDATES+=("$CARGO_TARGET_ROOT/release/orca" "$CARGO_TARGET_ROOT/debug/orca")
    fi
    BINARY_CANDIDATES+=("$REPO_ROOT/target/release/orca" "$REPO_ROOT/target/debug/orca")
    ...
    if [[ -z "$BINARY" ]] && command -v orca &> /dev/null; then
        BINARY="$(command -v orca)"
    fi

    if [[ -z "$BINARY" ]]; then
        echo -e "${RED}Error: orca binary not found${NC}"
...
if ! $BINARY_EXPLICIT && [[ "$BINARY" == "$REPO_ROOT/"* ]]; then
    ...
    if [[ -n "$EXPECTED_VERSION" && "$ACTUAL_VERSION" != "$EXPECTED_VERSION" ]]; then
        echo -e "${RED}Error: stale orca binary selected${NC}"
        echo "Binary: $BINARY"
...
# overridden via ORCA_PACKS. If we read ~/.config/orca/config.toml, results become
...
if ! $JSON_OUTPUT; then
    echo -e "${BOLD}${BLUE}orca End-to-End Test Suite${NC}"
...
has_orca_warning() {
    ...
    [[ -n "$stderr_output" ]] && echo "$stderr_output" | grep -q "orca WARNING"
}
...
test_command_with_packs() {
    ...
    # Run the binary with ORCA_PACKS environment variable.
    ...
    case "$expected" in
        block)
            ...
            ;;
        warn)
            if has_permission_decision "$result" "ask" && has_orca_warning "$err"; then
                log_pass "WARNED (pack=$packs): $desc"
                return 0
            fi
            log_fail "Should WARN with pack=$packs: $desc" "JSON with permissionDecision: ask; stderr contains orca WARNING" "stdout=${result:-<empty>} | stderr=${err:-<empty>}"
            return 0
            ;;
        allow)
            if [[ -z "$result" ]]; then
...
test_default_severity_behavior() {
    ...
    # Run WITHOUT ORCA_POLICY_DEFAULT_MODE to test default severity behavior
    echo "$encoded" | base64 -d | \
        HOME="$TEST_ENV_HOME" \
        XDG_CONFIG_HOME="$TEST_ENV_XDG" \
        ORCA_ALLOWLIST_SYSTEM_PATH="" \
    ...
    case "$expected" in
        warn)
            if has_permission_decision "$out" "ask" && has_orca_warning "$err"; then
                log_pass "WARNED (default severity): $desc"
            else
                log_fail "Should WARN (default severity): $desc" "JSON with permissionDecision: ask; stderr contains orca WARNING" "stdout=${out:-<empty>} | stderr=${err:-<empty>}"
            fi
            ;;
        *)
...
# Test helper: run command WITH explicit ORCA_POLICY_DEFAULT_MODE and check stdout/stderr
...
test_command_with_policy() {
    ...
    echo "$encoded" | base64 -d | \
        ...
        XDG_CONFIG_HOME="$TEST_ENV_XDG" \
        ORCA_ALLOWLIST_SYSTEM_PATH="" \
        ORCA_POLICY_DEFAULT_MODE="$policy_mode" \
        "$BINARY" >"$out_file" 2>"$err_file" || true
    ...
    case "$expected" in
        ...
        warn)
            if has_permission_decision "$out" "ask" && has_orca_warning "$err"; then
                ...
                log_fail "Should WARN (policy=$policy_mode): $desc" "JSON with permissionDecision: ask; stderr contains orca WARNING" "stdout=${out:-<empty>} | stderr=${err:-<empty>}"
            fi
            ;;
        silent)
            if [[ -z "$out" ]] && [[ -z "$err" ]]; then
...
PY
    ...
    local badge_url="https://codecov.io/gh/christopherkarani/Orca/branch/main/graph/badge.svg"
...
# These use the new test helper that doesn't set ORCA_POLICY_DEFAULT_MODE
...
test_command_with_layered_allowlists() {
    ...
    if [[ -n "$project_allowlist" ]]; then
        mkdir -p "$project_dir/.orca"
        echo "$project_allowlist" > "$project_dir/.orca/allowlist.toml"
    fi

    if [[ -n "$user_allowlist" ]]; then
        mkdir -p "$user_config_dir/orca"
        echo "$user_allowlist" > "$user_config_dir/orca/allowlist.toml"
    fi
    ...
    result=$(cd "$project_dir" && echo "$encoded" | base64 -d | env \
        ...
        ORCA_ALLOWLIST_SYSTEM_PATH="$system_path" \
        $env_vars "$BINARY" 2>/dev/null || true)
