...
if [[ -z "$BINARY" ]]; then
    # Try to find orca in common build locations relative to the script or current dir
    ...
    if [[ -f "$PROJECT_ROOT/target/release/orca" ]]; then 
        BINARY="$PROJECT_ROOT/target/release/orca"
    elif [[ -f "$PROJECT_ROOT/target/debug/orca" ]]; then 
        BINARY="$PROJECT_ROOT/target/debug/orca"
    elif command -v orca &>/dev/null; then 
        BINARY="orca"
    else 
        echo "Error: orca binary not found. Build it with 'cargo build --release' or use --binary"
        exit 1
    fi
fi
...
test_cmd() {
    ...
    # Run orca with the pack enabled
    # We force enable the pack to ensure it's active even if config doesn't have it
    local out
    out=$(echo "$encoded" | base64 -d | ORCA_PACKS="$PACK_NAME" "$BINARY" 2>&1 || true)
