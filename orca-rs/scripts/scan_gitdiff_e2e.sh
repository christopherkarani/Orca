#!/bin/bash
#
# End-to-End Test Script for `orca scan --git-diff`
#
...
#   - Runs `orca scan --git-diff` and asserts correct output/exit code
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --binary, -b    Path to orca binary"
...
if [[ -z "$BINARY" ]]; then
    if command -v orca &> /dev/null; then
        BINARY="orca"
    elif [[ -f "./target/release/orca" ]]; then
        BINARY="./target/release/orca"
    elif [[ -f "./target/debug/orca" ]]; then
        BINARY="./target/debug/orca"
    else
...
echo -e "${BOLD}${BLUE}orca scan --git-diff E2E Test Suite${NC}"
