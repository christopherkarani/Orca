...
/// Path to the ORCA binary (uses same target directory as the test binary).
...
fn test_test_command_json_valid() {
    let (stdout, stderr, exit_code) = run_orca(&["test", "--format", "json", "git status"]);
...
fn test_test_command_json_allowed() {
    let (stdout, _stderr, _) = run_orca(&["test", "--format", "json", "echo hello"]);
...
fn test_test_command_json_denied() {
    let (stdout, _stderr, _) = run_orca(&["test", "--format", "json", "git reset --hard"]);
...
fn test_explain_command_json_valid() {
    let (stdout, stderr, exit_code) =
        run_orca(&["explain", "--format", "json", "git reset --hard"]);
...
fn test_explain_command_json_has_trace() {
    let (stdout, _stderr, _) = run_orca(&["explain", "--format", "json", "git reset --hard"]);
...
fn test_packs_command_json_valid() {
    let (stdout, stderr, exit_code) = run_orca(&["packs", "--format", "json"]);
...
fn test_packs_command_json_pack_structure() {
    let (stdout, _stderr, _) = run_orca(&["packs", "--format", "json"]);
...
fn test_scan_command_json_valid() {
    ...
    let temp_dir = std::env::temp_dir().join("orca_test_scan");
    let _ = std::fs::create_dir_all(&temp_dir);
    let test_file = temp_dir.join("test.sh");
    std::fs::write(&test_file, "#!/bin/bash\necho hello\n").ok();

    let (stdout, stderr, exit_code) = run_orca(&[
        "scan",
        "--format",
        "json",
        "--paths",
...
fn test_all_json_outputs_are_objects() {
    ...
    for cmd_args in commands {
        let (stdout, stderr, exit_code) = run_orca(&cmd_args.iter().copied().collect::<Vec<_>>());
...
fn test_json_outputs_parseable_by_jq() {
    ...
    for cmd_args in commands {
        let (stdout, _stderr, _) = run_orca(&cmd_args.iter().copied().collect::<Vec<_>>());
...
fn test_decision_values_are_lowercase() {
    ...
    for (cmd, expected_decision) in commands {
        let (stdout, _stderr, _) = run_orca(&["test", "--format", "json", cmd]);
