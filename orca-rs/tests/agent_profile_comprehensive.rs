...
/// Path to the ORCA binary. Prefers `CARGO_BIN_EXE_orca` (set by Cargo when
/// building integration tests against a binary target) and falls back to the
/// `target/<profile>/orca` layout used by `cargo test`.
fn orca_binary() -> std::path::PathBuf {
    if let Some(p) = option_env!("CARGO_BIN_EXE_orca") {
...
/// Per-spawn isolated HOME / XDG_CONFIG_HOME / TMPDIR so orca cannot read or
...
fn make_isolated_home() -> tempfile::TempDir {
    ...
    std::fs::create_dir_all(dir.path().join(".config/orca"))
        .expect("failed to create isolated XDG_CONFIG_HOME/orca");
    dir
}
...
/// Run ORCA in hook mode with environment variables.
///
/// Spawns orca in a hermetic environment (isolated HOME/XDG_CONFIG_HOME/TMPDIR)
...
/// Run ORCA in robot mode for cleaner JSON output.
///
/// Spawns orca in a hermetic environment (isolated HOME/XDG_CONFIG_HOME/TMPDIR)
/// so the test cannot accidentally read the developer's real config or write
/// to their real history database.
...
/// Run ORCA test command with agent type.
#[allow(dead_code)]
...
mod agent_detection_tests {
    ...
    fn test_detects_claude_code_via_env() {
        ...
        assert!(
            stderr.contains("orca") || !stderr.is_empty(),
...
mod profile_loading_tests {
    ...
    fn test_loads_agent_profile_from_config() {
        ...
        // by checking that ORCA runs without errors when a config is present
...
mod trust_level_tests {
    ...
    fn test_trust_level_ordering() {
        ...
        // the ORCA binary understands trust levels in configuration
        let (stdout, stderr, exit_code) = run_robot_mode_with_env(&["config"], &[]);
...
mod unknown_agent_tests {
    ...
    fn test_unknown_agent_uses_safe_defaults() {
        // When agent is unknown, ORCA should use conservative defaults
    ...
    fn test_custom_agent_name_handled() {
        ...
        let (stdout, _stderr, exit_code) = run_robot_mode_with_env(
            &["test", "git status"],
            &[("ORCA_AGENT_TYPE", "my-custom-agent")],
        );
