...
fn run_hook_mode_raw(input: &str) -> (String, String, i32) {
    let mut child = Command::new(orca_binary())
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("failed to spawn orca process");
...
fn test_test_command_exit_0() {
    let output = Command::new(orca_binary())
        ...
        .output()
        .expect("failed to run orca test");

    assert!(output.status.success(), "orca test <safe> should exit 0");
}
...
fn test_test_command_deny_exit_1() {
    ...
    assert_eq!(
        output.status.code(),
        Some(1),
        "orca test <dangerous> should exit 1"
    );
}
...
fn test_explain_command_exit_0() {
    let output = Command::new(orca_binary())
        ...
        .output()
        .expect("failed to run orca explain");

    assert!(output.status.success(), "orca explain should exit 0");
}
...
fn test_packs_command_exit_0() {
    let output = Command::new(orca_binary())
        .args(["packs"])
        .output()
        .expect("failed to run orca packs");

    assert!(output.status.success(), "orca packs should exit 0");
}
...
fn test_version_exit_0() {
    let output = Command::new(orca_binary())
        .args(["--version"])
        .output()
        .expect("failed to run orca --version");

    assert!(output.status.success(), "orca --version should exit 0");
}
...
fn test_help_exit_0() {
    let output = Command::new(orca_binary())
        .args(["--help"])
        .output()
        .expect("failed to run orca --help");

    assert!(output.status.success(), "orca --help should exit 0");
}
