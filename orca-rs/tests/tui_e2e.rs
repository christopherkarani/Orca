...
/// Run orca in hook mode with the given command and environment variables.
...
/// Run orca with CLI arguments and environment variables.
fn run_orca_with_env(args: &[&str], env_vars: &[(&str, &str)]) -> (String, String, i32) {
    let temp = tempfile::tempdir().expect("failed to create temp dir");
    ...
    let output = cmd.output().expect("failed to execute orca");
...
mod ci_fallback_tests {
    ...
    fn ci_env_with_explain_command() {
        ...
        let (stdout, stderr, exit_code) =
            run_orca_with_env(&["explain", "git reset --hard"], &[("CI", "true")]);
...
mod no_color_tests {
    ...
    fn no_color_with_scan_command() {
        ...
        let (stdout, stderr, _) = run_orca_with_env(
...
mod term_dumb_tests {
    ...
    fn term_dumb_with_explain() {
        let (stdout, stderr, exit_code) =
            run_orca_with_env(&["explain", "git reset --hard"], &[("TERM", "dumb")]);
...
mod json_format_tests {
    ...
    fn json_format_is_pure_json() {
        let (stdout, _, exit_code) =
            run_orca_with_env(&["explain", "--format", "json", "git reset --hard"], &[]);
    ...
    fn json_format_scan_is_valid() {
        ...
        let (stdout, stderr, exit_code) = run_orca_with_env(
...
mod denial_content_tests {
    ...
    fn denial_has_allow_once_info() {
        ...
        if let Some(remediation) = hook_output.get("remediation") {
            ...
            assert!(
                allow_cmd.contains("orca allow-once"),
                "remediation should contain orca allow-once command"
            );
        }
    }
}
