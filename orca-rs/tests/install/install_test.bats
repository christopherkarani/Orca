...
@test "check_installed_version: returns 1 when orca not installed" {
    log_test "Testing version check when orca not installed..."
...
@test "check_installed_version: returns 0 when versions match" {
    ...
    # Create mock orca binary that returns version
...
@test "check_installed_version: returns 1 when versions differ" {
    ...
    # Create mock orca binary that returns different version
    cat > "$DEST/orca" << 'MOCKEOF'
#!/bin/bash
echo "orca 1.0.0"
MOCKEOF
...
@test "check_installed_version: normalizes v prefix" {
    ...
    # Create mock orca binary that returns version without v prefix
    cat > "$DEST/orca" << 'MOCKEOF'
#!/bin/bash
echo "orca 1.2.3"
MOCKEOF
