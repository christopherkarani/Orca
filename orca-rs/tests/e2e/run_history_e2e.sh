...
#
# This script runs comprehensive end-to-end tests for the ORCA history system.
...
TEMP_DIR=$(mktemp -d -t orca_history_e2e_XXXXXX)
...
EOF
    ...
    export ORCA_HISTORY_DB="$HISTORY_DB"
...
test_database_creation() {
    ...
    echo "$input" | "$ORCA_BIN" 2>/dev/null || true
...
test_command_logging() {
    ...
    for cmd in "${commands[@]}"; do
        echo "$cmd" | "$ORCA_BIN" 2>/dev/null || true
    done
...
main() {
    echo ""
    echo "=============================================="
    echo "  ORCA History E2E Test Suite"
    echo "=============================================="
    echo ""
