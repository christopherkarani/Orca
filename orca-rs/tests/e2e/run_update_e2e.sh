#!/bin/bash
#
# E2E tests for ORCA self-update mechanism
#
...
#
# Requirements:
# - orca binary built (cargo build --release)
# - bash 4.0+
#
# Exit codes:
#   0 - All tests passed
...
setup_test_env() {
    ...
    mkdir -p "$TEST_HOME/.local/share/orca/backups"
    mkdir -p "$TEST_HOME/.cache/orca"

    # Copy orca binary to test environment
    cp "$ORCA_BINARY" "$TEST_HOME/.local/bin/orca"
    chmod +x "$TEST_HOME/.local/bin/orca"
...
build_binary() {
    ...
    if cargo build --release --bin orca 2>&1 | tee -a "$LOG_FILE" | tail -3; then
        ORCA_BINARY="$PROJECT_ROOT/target/release/orca"
        if [[ ! -f "$ORCA_BINARY" ]]; then
            echo "Binary not found at $ORCA_BINARY"
            exit 1
        fi
        log "Build complete."
    else
...
test_version_display() {
    ...
    local output
    output=$("$TEST_HOME/.local/bin/orca" --version 2>&1 || true)
    log_verbose "Version output: $output"

    if echo "$output" | grep -qE "orca [0-9]+\.[0-9]+\.[0-9]+"; then
...
test_update_check() {
    ...
    local output
    local exit_code=0
    output=$("$TEST_HOME/.local/bin/orca" update --check 2>&1) || exit_code=$?
...
test_update_check_json() {
    ...
    local output
    local exit_code=0
    output=$("$TEST_HOME/.local/bin/orca" update --check --json 2>&1) || exit_code=$?
...
test_backup_list() {
    ...
    # Create mock backups
    local backup_dir="$TEST_HOME/.local/share/orca/backups"
    mkdir -p "$backup_dir"

    # Create mock backup files
    echo "mock binary v1" > "$backup_dir/orca-0.2.11-1737100000"
    cat > "$backup_dir/orca-0.2.11-1737100000.json" << 'EOF'
{
  "version": "0.2.11",
  "created_at": 1737100000,
  "original_path": "/usr/local/bin/orca"
}
EOF

    echo "mock binary v2" > "$backup_dir/orca-0.2.12-1737200000"
    cat > "$backup_dir/orca-0.2.12-1737200000.json" << 'EOF'
{
  "version": "0.2.12",
...
EOF
    ...
    local output
    local exit_code=0
    output=$("$TEST_HOME/.local/bin/orca" update --list-backups 2>&1) || exit_code=$?
...
test_empty_backup_list() {
    ...
    rm -rf "$TEST_HOME/.local/share/orca/backups"/*
...
test_invalid_version() {
    ...
    local output
    local exit_code=0
    output=$("$TEST_HOME/.local/bin/orca" update --version "99.99.99" 2>&1) || exit_code=$?
...
test_help_includes_update() {
    ...
    local output
    output=$("$TEST_HOME/.local/bin/orca" --help 2>&1 || true)
...
test_update_help() {
    ...
    local output
    output=$("$TEST_HOME/.local/bin/orca" update --help 2>&1 || true)
...
test_rollback_no_backups() {
    ...
    local output
    local exit_code=0
    output=$("$TEST_HOME/.local/bin/orca" update --rollback 2>&1) || exit_code=$?
...
# Test 10: ORCA_NO_UPDATE_CHECK environment variable
test_update_check_disabled() {
    ...
    export ORCA_NO_UPDATE_CHECK=1

    # The binary should respect this - exact behavior depends on implementation
    # Just verify it doesn't crash
    local output
    local exit_code=0
    output=$("$TEST_HOME/.local/bin/orca" --version 2>&1) || exit_code=$?

    unset ORCA_NO_UPDATE_CHECK

    if [[ $exit_code -eq 0 ]]; then
        pass "ORCA_NO_UPDATE_CHECK env var does not crash binary"
    else
        fail "ORCA_NO_UPDATE_CHECK env var does not crash binary" "Exit code: $exit_code"
    fi
...
test_same_version_no_change() {
    ...
    current_version=$("$TEST_HOME/.local/bin/orca" --version 2>&1 | grep -oP "[0-9]+\.[0-9]+\.[0-9]+" | head -1 || echo "")
    ...
    # Get hash before
    local before_hash
    before_hash=$(sha256sum "$TEST_HOME/.local/bin/orca" | cut -d' ' -f1)
    log_verbose "Before hash: $before_hash"

    # Attempt update to same version (should be no-op or graceful message)
    local output
    output=$("$TEST_HOME/.local/bin/orca" update --version "$current_version" 2>&1 || true)
    log_verbose "Same version update output: $output"

    # Get hash after
    local after_hash
    after_hash=$(sha256sum "$TEST_HOME/.local/bin/orca" | cut -d' ' -f1)
...
main() {
    echo "=== ORCA Update E2E Test Suite ==="
    echo "Started: $(date)"
    ...
    echo ""
    echo "[10/11] Testing ORCA_NO_UPDATE_CHECK..."
    test_update_check_disabled
