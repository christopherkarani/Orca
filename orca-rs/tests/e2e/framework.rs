...
//! and running ORCA commands with detailed output capture.
...
/// Result of a ORCA command execution.
#[derive(Debug, Clone)]
...
impl E2ETestContextBuilder {
    ...
    pub fn build(self) -> E2ETestContext {
        ...
        let orca_dir = temp_path.join(".orca");
        std::fs::create_dir_all(&orca_dir).expect("Failed to create .orca directory");
        ...
        env_vars.insert("ORCA_ALLOWLIST_SYSTEM_PATH".to_string(), String::new());

        if let Some(packs) = &self.packs {
            env_vars.insert("ORCA_PACKS".to_string(), packs.clone());
        }

        if let Some(agent) = &self.agent_type {
            env_vars.insert("ORCA_AGENT_TYPE".to_string(), agent.clone());
        }

        // Find the ORCA binary
        let binary_path = find_orca_binary();
...
impl E2ETestContext {
    ...
    /// Run ORCA in hook mode with a command.
    ///
    /// This simulates the Claude Code hook protocol by sending JSON input to stdin.
    #[must_use]
    pub fn run_orca_hook(&self, command: &str) -> OrcaOutput {
        let json_input = format!(
            r#"{{"tool_name":"Bash","tool_input":{{"command":"{}"}}}}"#,
            escape_json_string(command)
        );

        self.run_orca_with_stdin(&json_input, &[])
    }

    /// Run ORCA with custom arguments.
    #[must_use]
    pub fn run_orca(&self, args: &[&str]) -> OrcaOutput {
        self.run_orca_with_stdin("", args)
    }

    /// Run ORCA with stdin input and arguments.
    #[must_use]
    pub fn run_orca_with_stdin(&self, stdin: &str, args: &[&str]) -> OrcaOutput {
    ...
    pub fn assert_blocked(&self, output: &OrcaOutput) {
        assert!(
            output.is_blocked(),
    ...
    pub fn assert_allowed(&self, output: &OrcaOutput) {
        assert!(
            output.is_allowed(),
    ...
    pub fn assert_blocked_by_rule(&self, output: &OrcaOutput, rule_id: &str) {
...
/// Find the ORCA binary, preferring local build artifacts.
fn find_orca_binary() -> PathBuf {
    // Cargo integration tests expose built binaries via CARGO_BIN_EXE_* env vars.
    for env_var in [
        "CARGO_BIN_EXE_orca-daemon",
        "CARGO_BIN_EXE_orca",
        "CARGO_BIN_EXE_orca_rs",
    ] {
    ...
    let release_path = PathBuf::from("./target/release/orca-daemon");
    if release_path.exists() {
        return std::fs::canonicalize(&release_path).unwrap_or(release_path);
    }

    let debug_path = PathBuf::from("./target/debug/orca-daemon");
    if debug_path.exists() {
        return std::fs::canonicalize(&debug_path).unwrap_or(debug_path);
    }

    // Legacy binary names (for backwards compatibility)
    let release_path_legacy = PathBuf::from("./target/release/orca");
    if release_path_legacy.exists() {
        return std::fs::canonicalize(&release_path_legacy).unwrap_or(release_path_legacy);
    }

    let debug_path_legacy = PathBuf::from("./target/debug/orca");
    if debug_path_legacy.exists() {
        return std::fs::canonicalize(&debug_path_legacy).unwrap_or(debug_path_legacy);
    }

    // Fall back to PATH
    which::which("orca-daemon")
        .or_else(|_| which::which("orca"))
        .unwrap_or_else(|_| PathBuf::from("orca-daemon"))
}
...
/// Minimal configuration - bare minimum for ORCA to function.
const MINIMAL_CONFIG: &str = r#"# Minimal ORCA Configuration
...
const FULL_FEATURED_CONFIG: &str = r#"# Full-Featured ORCA Configuration
...
const PATH_SPECIFIC_CONFIG: &str = r#"# Path-Specific ORCA Configuration
...
const TEMPORARY_RULES_CONFIG: &str = r#"# Temporary Rules ORCA Configuration
...
const AGENT_PROFILES_CONFIG: &str = r#"# Agent Profiles ORCA Configuration
...
const GIT_AWARENESS_CONFIG: &str = r#"# Git Awareness ORCA Configuration
...
const GRADUATED_RESPONSE_CONFIG: &str = r#"# Graduated Response ORCA Configuration
