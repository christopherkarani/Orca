    ...
    "PreToolUse": [
      {
        "matcher": "Bash",
        "hooks": [
          {"type": "command", "command": "$HOME/.local/bin/orca"}
        ]
      }
    ]
  }
...
test_version_comparison() {
    ...
    # Create mock orca with specific version
    cat > "$DEST/orca" << 'EOF'
#!/bin/bash
echo "orca 1.2.3"
EOF
...
return 0 2>/dev/null || true
    ...
    # Check orca was added
    if ! grep -q "orca" "$HOME/.claude/settings.json"; then
        fail "Settings merge preserves existing content" "orca hook not added"
        cleanup_test_env
        return
    fi
...
test_lock_mechanism() {
    ...
    local lock_dir="/tmp/orca-install.lock.d"
...
main() {
    echo "=== ORCA Install Script E2E Test Suite ==="
    echo "Started: $(date)"
