...
/// Run orca in hook mode with the given command as JSON input.
    ...
    if hook_output["permissionDecision"] == "deny" {
        ...
        if let Some(remediation) = hook_output.get("remediation") {
            ...
            assert!(
                ...
                "allowOnceCommand should contain 'orca allow-once'"
            );
            assert!(
...
fn test_hook_output_stderr_includes_allowlist_add_hint() {
    ...
    assert!(
        stderr.contains("orca allowlist add core.git:reset-hard --project"),
