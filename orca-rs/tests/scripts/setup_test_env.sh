#!/usr/bin/env bash
# setup_test_env.sh - Test Environment Setup for ORCA E2E Tests
# Part of bead [E0-T1] E2E test infrastructure
#
# This script prepares an isolated test environment for ORCA E2E testing.
...
TEST_ENV_ROOT="${TEST_ENV_ROOT:-/tmp/orca-e2e-tests}"
...
Environment Variables:
    ...
    ORCA_BINARY          Path to orca binary
...
create_env() {
    ...
    cat > "$TEST_ENV_DIR/.orca-test-env" << EOF
{
    "id": "$TEST_ENV_ID",
...
init_git_repo() {
    ...
    git config user.email "test@orca-e2e.local"
    git config user.name "ORCA E2E Test"

    # Create initial content
    cat > README.md << 'EOF'
# ORCA Test Repository

This is a mock repository for ORCA E2E testing.
EOF
...
EOF
    ...
    cat > src/main.rs << 'EOF'
fn main() {
    println!("Hello from ORCA test repo!");
}
EOF
...
build_orca() {
    ...
    if [[ "$release" == "true" ]]; then
        ...
        cargo build -q
        ORCA_BINARY="$PROJECT_ROOT/target/debug/orca"
    fi

    if [[ ! -x "$ORCA_BINARY" ]]; then
        log_error "Failed to build orca"
        return 1
    fi

    # Symlink to test env
    ln -sf "$ORCA_BINARY" "$TEST_ENV_DIR/orca"

    log_success "Built: $ORCA_BINARY"
...
export ORCA_TEST_WORKSPACE="$TEST_ENV_DIR/workspace"
export ORCA_TEST_CONFIGS="$TEST_ENV_DIR/configs"
export ORCA_TEST_COMMANDS="$TEST_ENV_DIR/commands"
export ORCA_BINARY="${ORCA_BINARY:-$TEST_ENV_DIR/orca}"
...
destroy_env() {
    ...
    # Safety check - ensure it's a ORCA test env
    if [[ ! -f "$env_dir/.orca-test-env" ]]; then
        log_error "Not a ORCA test environment: $env_dir"
        return 1
    fi
...
list_envs() {
    ...
    for env in "$TEST_ENV_ROOT"/*/; do
        ...
        local env_meta="$env/.orca-test-env"
...
validate_env() {
    ...
    # Check ORCA binary
    if [[ -L "$env_dir/orca" ]] && [[ -x "$env_dir/orca" ]]; then
        local version=$("$env_dir/orca" --version 2>/dev/null || echo "unknown")
        log_info "ORCA: $version"
    else
        log_warn "ORCA binary not linked"
    fi
