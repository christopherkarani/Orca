#!/usr/bin/env bash
# run_e2e_tests.sh - Master E2E Test Runner for ORCA
# Part of bead [E0-T1] E2E test infrastructure
#
# This script orchestrates all E2E tests for the Orca.
# It builds orca, sets up the test environment, runs tests, and generates reports.
...
Options:
    ...
    --no-build          Skip building ORCA (use existing binary)
...
Environment:
    ORCA_BINARY          Path to orca binary (overrides build)
    ORCA_TEST_VERBOSE    Enable verbose test output
    ORCA_REPORT_DIR      Directory for test reports
...
preflight_checks() {
    ...
    if [[ ! -f "$PROJECT_ROOT/Cargo.toml" ]]; then
        log_error "Not in ORCA project root"
        checks_passed=false
    else
...
build_orca() {
    ...
    # Set binary path
    if [[ "$RELEASE_MODE" == "true" ]]; then
        ORCA_BINARY="${ORCA_BINARY:-$PROJECT_ROOT/target/release/orca}"
    else
        ORCA_BINARY="${ORCA_BINARY:-$PROJECT_ROOT/target/debug/orca}"
    fi

    if [[ ! -x "$ORCA_BINARY" ]]; then
        log_error "ORCA binary not found: $ORCA_BINARY"
        exit 1
    fi
...
run_fixture_tests() {
    ...
    for cmd_file in "$FIXTURES_DIR/commands"/*.txt; do
        ...
        while IFS= read -r line || [[ -n "$line" ]]; do
            ...
            if result=$(echo "$hook_input" | timeout "$TIMEOUT" "$ORCA_BINARY" check 2>&1); then
                # ORCA allowed (exit 0 or empty output)
                ...
                # ORCA blocked (exit non-zero or denial output)
        ...
        cat > "${report_file}.json" << EOF
{
    ...
    "orca_version": "$(cat "$PROJECT_ROOT/Cargo.toml" | grep '^version' | head -1 | cut -d'"' -f2)",
...
# ORCA E2E Test Report

**Generated:** $(date)
**ORCA Version:** $(cat "$PROJECT_ROOT/Cargo.toml" | grep '^version' | head -1 | cut -d'"' -f2)
...
main() {
    ...
    if [[ "$QUIET" != "true" ]]; then
        echo -e "${CYAN}"
        echo "╔════════════════════════════════════════════════════════════════╗"
        echo "║         ORCA E2E Test Runner                                    ║"
        echo "║         Orca - End-to-End Tests          ║"
        echo "╚════════════════════════════════════════════════════════════════╝"
        echo -e "${NC}"
    fi
