...
use orca_rs::packs::{Pack, PackRegistry};
...
fn generate_all_docs() -> std::io::Result<()> {
    ...
    index.push_str(
        "This directory contains detailed reference documentation for all orca packs.\n\n",
    );
    index.push_str("## Quick Start\n\n");
    index.push_str("Enable packs in `~/.config/orca/config.toml`:\n\n");
    index.push_str("```toml\n");
