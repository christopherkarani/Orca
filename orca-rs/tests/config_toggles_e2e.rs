...
impl TestEnv {
    ...
    /// Run orca in hook mode with the given command.
    ...
    /// Run orca test command (non-hook mode).
    fn run_test_command(&self, command: &str) -> HookOutput {
        ...
        let output = cmd.output().expect("failed to execute orca test");
...
fn test_cli_test_mode_with_toggles() {
    ...
    // orca test returns non-zero for denied commands
    assert!(
