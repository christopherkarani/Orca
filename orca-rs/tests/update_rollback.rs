...
fn create_mock_backup_entry(
    ...
    let backup_name = format!("orca-{version}-{timestamp}");
...
fn test_backup_entry_metadata_format() {
    ...
    let metadata_path = backup_dir.join("orca-0.2.12-1737200000.json");
...
fn test_backup_binary_permissions() {
    let temp_dir = TempDir::new().unwrap();
    let binary_path = temp_dir.path().join("orca");
...
fn test_atomic_binary_replacement() {
    ...
    let temp_path = temp_dir.path().join("orca.new");
...
fn test_rollback_to_specific_version() {
    ...
    let backup_binary = backup_dir.join("orca-1.9.0-1737100000");
...
fn test_rollback_to_most_recent_backup() {
    ...
    for entry in fs::read_dir(&backup_dir).unwrap().filter_map(Result::ok) {
        ...
        if path.extension().is_some_and(|ext| ext == "json") {
            ...
            let binary_name = format!("orca-{version}-{timestamp}");
...
fn test_rollback_preserves_config() {
    let temp_dir = TempDir::new().unwrap();
    let config_dir = temp_dir.path().join(".config").join("orca");
