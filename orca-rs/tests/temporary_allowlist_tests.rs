...
mod duration_parsing {
    use orca_rs::allowlist::parse_duration;
...
mod timestamp_parsing {
    use orca_rs::allowlist::{AllowEntry, AllowSelector, RuleId, is_expired};
...
mod expiration_exclusivity {
    use orca_rs::allowlist::validate_expiration_exclusivity;
...
fn e2e_allowlist_list_shows_expiration_info() {
    ...
    let output = ctx.run_orca(&["allowlist", "list"]);
    let combined = format!("{}{}", output.stdout, output.stderr);
...
fn e2e_expired_entry_does_not_allow_command() {
    ...
    let output = ctx.run_orca_hook("git reset --hard HEAD");
...
fn e2e_validate_checks_expired_entries() {
    ...
    let output = ctx.run_orca(&["allowlist", "validate"]);
...
fn integration_entry_validity_check() {
    use orca_rs::allowlist::{AllowEntry, AllowSelector, RuleId, is_entry_valid};
    use std::collections::HashMap;
