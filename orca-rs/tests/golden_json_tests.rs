...
fn mask_dynamic_fields(mut json: Value) -> Value {
    if json.get("orca_version").is_some() {
        json["orca_version"] = Value::String("<DYNAMIC_VERSION>".to_string());
    }
        ...
        if let Some(remediation) = hook_output.get_mut("remediation") {
            if remediation.get("allowOnceCommand").is_some() {
                remediation["allowOnceCommand"] =
                    Value::String("orca allow-once <DYNAMIC>".to_string());
            }
        }
    }
    json
...
fn golden_hook_deny_filesystem() {
    ...
    let output = ctx.run_orca_hook("rm -rf /");
...
fn golden_hook_deny_git_force_push() {
    ...
    let output = ctx.run_orca_hook("git push --force origin main");
...
fn golden_hook_allow_simple() {
    ...
    let output = ctx.run_orca_hook("echo hello");
...
fn golden_hook_allow_git_status() {
    ...
    let output = ctx.run_orca_hook("git status");
...
fn golden_hook_allow_git_checkout_branch() {
    ...
    let output = ctx.run_orca_hook("git checkout -b new-feature");
...
fn golden_json_permission_decision_values() {
    ...
    let denied = ctx.run_orca_hook("rm -rf /");
    ...
    let allowed = ctx.run_orca_hook("echo hello");
    assert!(
...
fn golden_json_severity_values() {
    ...
    let critical = ctx.run_orca_hook("rm -rf /");
...
/// Run orca in robot mode via test subcommand
fn run_robot_mode(command: &str) -> Option<Value> {
    ...
    let output = Command::new(&orca_bin)
