//! End-to-end tests for stdin batch mode (`orca hook --batch`).
//!
...
/// Run orca in batch hook mode with the given JSONL input.
fn run_orca_batch(input: &str) -> std::process::Output {
    run_orca_batch_with_args(input, &[])
}

/// Run orca in batch hook mode with additional CLI arguments.
fn run_orca_batch_with_args(input: &str, extra_args: &[&str]) -> std::process::Output {
    ...
    let mut child = cmd.spawn().expect("failed to spawn orca batch mode");
...
"#;
    ...
    let output = run_orca_batch(input);
    assert!(
...
"#;
    ...
    let output = run_orca_batch_with_args(input, &["--continue-on-error"]);
    assert!(
        output.status.success(),
...
"#;
    ...
    // Non-Bash tools should be skipped (not evaluated by orca)
...
"#;
    ...
    let output = run_orca_batch_with_args(input, &["--parallel"]);
    assert!(output.status.success());
