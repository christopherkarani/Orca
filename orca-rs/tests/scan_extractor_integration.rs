...
/// Run orca scan command and return output
fn run_orca_scan(args: &[&str]) -> std::process::Output {
    let orca_bin = std::env::var("ORCA_BIN")
        .map(std::path::PathBuf::from)
        .or_else(|_| {
            std::env::var_os("CARGO_BIN_EXE_orca")
                .map(std::path::PathBuf::from)
...
"#,
    ...
    let output = run_orca_scan(&["--paths", dir.path().to_str().unwrap(), "--format", "json"]);
