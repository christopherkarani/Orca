...
//! by orca (rules `core.git:checkout-discard` and `core.git:restore-worktree`),
...
//!    dangerous mistake — so orca allows them with an informational note
//!    to stderr instead of a hard block.
//!
//! 2. **Explicit permit cookie (opt-in, short-lived).** The agent (or user)
//!    runs `orca rebase-recover`, which writes a timestamp file into
//!    `.orca/rebase-recovery-permit`. For the next 120 seconds (or until the
...
//! ## Safety
//!
//! - The permit is scoped to the current repository's `.orca/` directory.
...
pub enum RecoveryReason {
    ...
    RebaseInProgress,
    /// A time-bounded permit issued by `orca rebase-recover` was valid.
...
/// directory. Located inside `.orca/` so it lives alongside other orca state
...
fn permit_path(cwd: &Path) -> PathBuf {
    ...
    anchor.join(".orca").join("rebase-recovery-permit")
}
...
/// # Errors
///
/// Returns an IO error if the `.orca/` directory cannot be created or the
