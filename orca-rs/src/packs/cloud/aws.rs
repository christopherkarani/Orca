    ...
    vec![
        ...
        destructive_pattern!(
            ...
            "Athena `start-query-execution --query-string file://…` loads the\n\
             SQL from disk, so ORCA can't inspect the statement. The file may\n\
        ...
        destructive_pattern!(
            ...
            "`--cli-input-json file://…` (or `-yaml`) supplies the whole\n\
             invocation — including QueryString — from a file on disk. ORCA\n\
             ...
             allowed because ORCA can read the literal in the command line\n\
...
mod tests {
    ...
    fn existing_aws_patterns_also_match_with_global_flags() {
        ...
        // silently exempt from ORCA protection unless we fix them all.
        let pack = create_pack();
        // ec2 terminate
        assert_blocks(
            &pack,
