...
#[command(after_help = "Run 'orca doctor' to verify your installation.")]
pub struct Cli {
...
pub enum Command {
    ...
    Doctor {
        ...
        #[arg(long, short, value_enum, default_value_t = DoctorFormat::Pretty, env = "ORCA_FORMAT")]
        format: DoctorFormat,
    },
    ...
    Install {
        ...
        /// Install the orca PreToolUse hook for Grok (xAI) at
        /// `~/.grok/hooks/orca.json` (user-level) or `./.grok/hooks/orca.json`
        /// (when combined with `--project`). Grok also picks up orca from
    ...
    ///
    /// Installs the orca hook into Claude Code settings (like `orca install`)
    ...
    /// Update orca to the latest release (re-runs the installer)
    #[command(name = "update")]
    ...
    ///
    /// Runs orca as an MCP (Model Context Protocol) server over stdio,
    /// allowing AI agents to integrate directly without shell hooks.
    ///
    /// Tools exposed:
    /// - `check_command`: Evaluate a command using orca policy
    /// - `scan_file`: Scan a file or directory for destructive commands
    /// - `explain_pattern`: Explain a orca rule by `rule_id`
    ///
    /// Example agent configuration (Claude Code):
    /// ```json
    /// {
    ///   "mcpServers": {
    ///     "orca": { "command": "orca", "args": ["mcp-server"] }
    ///   }
    /// }
    /// ```
...
/// Options for self-updating orca via the installer scripts.
#[derive(Args, Debug)]
...
pub struct UpdateCommand {
    ...
    /// don't want `orca update` to re-add the hook to user-level settings or
    ...
    if stderr_is_tty {
        if let Some(cached) = crate::update::read_cached_check() {
            if cached.update_available {
                eprintln!(
                    "! A new version of orca is available: {} -> {}\n  Run `orca update` to upgrade",
...
fn print_auto_detect_results(
    ...
    if dry_run {
        println!(
            "{}",
            "Run 'orca init --auto' to write this configuration.".yellow()
        );
    }
}
...
fn generate_config_with_packs(packs: &[String]) -> String {
    ...
    let mut lines = vec![
        "# orca configuration".to_string(),
        "# https://github.com/christopherkarani/Orca".to_string(),
        "# Generated by: orca init --auto".to_string(),
        String::new(),
        "[general]".to_string(),
...
/// Handle the `orca corpus` command.
...
fn handle_stats_command(
    ...
    if !log_path.exists() {
        ...
        println!();
        println!("To enable logging, add to your config (~/.config/orca/config.toml):");
        println!();
        println!("  [general]");
        println!("  log_file = \"~/.local/share/orca/blocked.log\"");
        println!();
...
/// Handle the `orca stats --rules` command.
fn handle_stats_rules(
    ...
    let db = match HistoryDb::open(db_path) {
        ...
        Err(err) => {
            ...
            if matches!(err, crate::history::HistoryError::Disabled) {
                ...
                println!();
                println!("To enable history, add to your config (~/.config/orca/config.toml):");
                println!();
                println!("  [history]");
    ...
    if metrics.is_empty() {
        ...
        println!("Run some commands through orca to generate metrics.");
        return Ok(());
    }
...
fn format_corpus_pretty(output: &CorpusOutput) -> String {
    ...
    let _ = writeln!(
        ...
        "{}\n",
        if colorize {
            "orca corpus".green().bold().to_string()
        } else {
            "orca corpus".to_string()
        }
    );
...
fn doctor_pretty(fix: bool) {
    ...
    println!("{}", "orca doctor".green().bold());
    println!();
    ...
    if which_orca().is_some() {
        println!("{}", "OK".green());
    } else {
        println!("{}", "NOT FOUND".red());
        issues += 1;
        println!("  orca binary not found in PATH");
    ...
    if !hook_diag.settings_exists {
    ...
    } else if hook_diag.orca_hook_count == 0 {
        ...
        if fix {
            ...
            println!("  → Run 'orca install' to register the hook");
        }
    } else if hook_diag.orca_hook_count > 1 {
        println!("{}", "WARNING".yellow());
        println!(
            "  Found {} orca hook entries (expected 1)",
            hook_diag.orca_hook_count
        );
        println!("  → Run 'orca uninstall && orca install' to fix duplicates");
        ...
        println!("  → orca must be a Bash hook, not other tool types");
        println!("  → Run 'orca uninstall && orca install' to fix");
    ...
    // — covered by Check 3 above). The native orca.json gives a cleaner doctor
    ...
    if grok_session_present || grok_home_exists {
        ...
        if user_hook_exists {
            ...
            if claude_compat_exists {
                println!(
                    "  Grok also picks up orca from {} (Claude compatibility layer).",
                    claude_compat_path.display()
                );
            }
        } else if claude_compat_exists && hook_diag.orca_hook_count >= 1 {
            ...
            println!(
                "  No native ~/.grok/hooks/orca.json — Grok will pick up orca from {}.",
                claude_compat_path.display()
            );
            println!(
                "  For a native install, run 'orca install --grok' (creates {}).",
                user_hook.display()
            );
        } else {
            ...
            if fix {
                ...
                println!("  → Run 'orca install --grok' to register the native hook");
                println!(
                    "    (or 'orca install' for the Claude-compat path at {})",
                    claude_compat_path.display()
                );
            }
        }
    ...
    match &config_diag.config_path {
        None => {
            ...
            if fix {
                ...
                println!("  → Run 'orca init -o ~/.config/orca/config.toml' to create one");
            }
        }
        ...
        Some(path) => {
            if config_diag.has_errors() || config_diag.has_warnings() {
                ...
                if !config_diag.unknown_packs.is_empty() {
                    ...
                    println!("  → Run 'orca packs list' to see available packs");
                }
    ...
    if let Some(observe_until) = config.policy().observe_until.as_ref() {
        ...
        if let Some(until) = observe_until.parsed_utc() {
            if &now < until {
                ...
                println!(
                    "  {} ORCA is now enforcing normal severity defaults",
                    "→".bold()
                );
    ...
    if allowlist_diag.total_errors > 0 {
        ...
        println!("  → Run 'orca allowlist validate' for details");
        ...
        println!("  → Use 'orca allow <rule-id> -r \"reason\"' to create one");
    } else {
        println!(
            "{} ({} layer{})",
            "OK".green(),
...
/// Rich terminal doctor output using OrcaConsole and markup.
#[cfg(feature = "rich-output")]
fn doctor_rich(fix: bool) {
    ...
    // Header
    con.rule(Some("[bold green] orca doctor [/]"));
    con.print("");
...
fn collect_doctor_report(fix: bool) -> DoctorReport {
    ...
    // Check 1: Binary in PATH
    let (status, message, remediation) = if which_orca().is_some() {
        (
            DoctorCheckStatus::Ok,
            "orca found in PATH".to_string(),
            None,
        )
    } else {
        issues += 1;
        (
            ...
            "orca binary not found in PATH".to_string(),
            Some("Run the install script or add orca to PATH".to_string()),
        )
    };
    ...
    let (status, message, remediation) = if !hook_diag.settings_exists {
        ...
        if fix {
            match install_hook_silent(false) {
                ...
                Ok(false) => (
                    ...
                    Some("Run 'orca install' to register the hook".to_string()),
                ),
                Err(e) => (
                    DoctorCheckStatus::Error,
            ...
            (
                DoctorCheckStatus::Error,
                "orca hook not registered".to_string(),
                Some("Run 'orca install' to register the hook".to_string()),
            )
        }
    } else if hook_diag.orca_hook_count > 1 {
        (
            ...
            Some("Run 'orca uninstall && orca install' to fix duplicates".to_string()),
        )
    } else if !hook_diag.wrong_matcher_hooks.is_empty() {
        issues += 1;
        (
            ...
            Some("orca must be a Bash hook; reinstall to fix".to_string()),
        )
    } else if !hook_diag.missing_executable_hooks.is_empty() {
        issues += 1;
        (
            ...
            Some("Run 'orca uninstall && orca install' to fix".to_string()),
        )
    } else {
        (
            DoctorCheckStatus::Ok,
            "orca hook registered".to_string(),
            None,
        )
    };
    checks.push(DoctorCheck {
        ...
        None => {
            if fix {
                    ...
                    match write_default_config() {
                        ...
                        Err(e) => {
                            ...
                            (
                                ...
                                Some("Create config with 'orca init'".to_string()),
                            )
                        }
                    }
                }
                ...
                (
                    ...
                    Some("Run 'orca init -o ~/.config/orca/config.toml'".to_string()),
                )
            }
        }
        Some(path) if config_diag.parse_error.is_some() => {
        ...
        Some(path) => {
            if config_diag.has_errors() || config_diag.has_warnings() {
                ...
                (
                    ...
                    Some("Run 'orca packs list' and fix invalid overrides".to_string()),
                )
            } else {
                (
                    DoctorCheckStatus::Ok,
    ...
    let (status, message, remediation) = if allowlist_diag.total_errors > 0 {
        ...
        (
            ...
            Some("Run 'orca allowlist validate' for details".to_string()),
        )
    } else if allowlist_diag.total_warnings > 0 {
        (
            DoctorCheckStatus::Warning,
        ...
        (
            ...
            Some("Use 'orca allow <rule-id> -r \"reason\"' to create one".to_string()),
        )
    } else {
        (
            DoctorCheckStatus::Ok,
        ...
        if !detections.is_empty() {
            ...
            let (status, message, remediation) = if not_enabled.is_empty() {
                ...
                (
                    ...
                    Some("Run 'orca init --auto' to enable detected packs".to_string()),
                )
            };
            checks.push(DoctorCheck {
                id: "auto_detect",
...
/// Install the orca hook entry into Claude Code settings without printing.
fn install_hook_silent(force: bool) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let changed = install_orca_hook_into_settings(&mut settings, force)?;
    if changed {
...
fn uninstall_orca_hook_from_settings(
    ...
    Ok(remove_orca_hooks_from_pre_tool_use(arr))
}

/// Install the orca hook entry into Claude Code settings.
///
/// When `project` is `true`, writes to `.claude/settings.json` inside the
/// current git repository root instead of the user-level
/// `~/.claude/settings.json`.
///
/// This is a wrapper around `install_orca_hook_into_settings` that handles the
...
/// Build the JSON body for a Grok `~/.grok/hooks/orca.json` hook file.
///
/// Resolves the orca binary path via `current_exe()` so the installed hook
/// always points at the same executable that was used to install it
/// (matching Claude's installer behavior). Falls back to bare `"orca"` when
...
/// Timeout matches orca's hook fast-path budget (well under 5s in practice).
fn build_grok_hook_config() -> serde_json::Value {
    let orca_cmd = std::env::current_exe()
        .ok()
        .and_then(|p| p.to_str().map(str::to_string))
        .unwrap_or_else(|| "orca".to_string());

    serde_json::json!({
        "description": "orca (Orca-rs) — blocks rm -rf, git reset --hard, force pushes, DROP DATABASE, kubectl delete, and similar destructive commands before Grok's run_terminal_cmd tool can execute them.",
        "hooks": {
            "PreToolUse": [
                {
                    "matcher": "Bash",
...
/// Install the orca hook into Grok's hook directory.
///
/// Writes a self-contained `orca.json` to `~/.grok/hooks/` (user-level) or
...
/// The shell snippet that checks whether the ORCA hook is still present in
...
const ORCA_SHELL_CHECK_SNIPPET: &str = r#"
...
/// Marker used to identify the ORCA shell check block for idempotent injection.
const ORCA_SHELL_CHECK_MARKER: &str = "# orca: warn if hook was silently removed";

/// Check whether a shell RC file already contains the ORCA startup check.
fn rc_has_orca_check(path: &std::path::Path) -> bool {
    if let Ok(content) = std::fs::read_to_string(path) {
        content.contains(ORCA_SHELL_CHECK_MARKER)
    } else {
        false
    }
}

/// Append the ORCA shell startup check to a shell RC file.
///
...
fn inject_shell_check(path: &std::path::Path) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    use std::io::Write;
    write!(file, "{}", ORCA_SHELL_CHECK_SNIPPET)?;
...
fn run_setup(
    ...
    // --- Step 1: Install the hook (same as `orca install`) ---
    ...
    let all_present = rc_files.iter().all(|p| rc_has_orca_check(p));
    if all_present {
        println!();
        println!(
            "{}",
    ...
    let should_inject = if auto_shell_check {
        ...
        println!("Claude Code can silently remove the orca hook when it rewrites settings.json.");
    ...
    if should_inject {
        ...
        println!(
            "{}",
            "Skipped shell startup check. You can add it later with: orca setup --shell-check"
                .dimmed()
        );
    }
...
/// Remove the orca hook entry from Claude Code settings.
///
/// This is a wrapper around `uninstall_orca_hook_from_settings` that handles the
/// file I/O and error reporting.
///
/// # Errors
///
...
fn uninstall_hook(purge: bool) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Remove orca hooks (fail if settings structure is unexpected).
    let removed = uninstall_orca_hook_from_settings(&mut settings)?;
    ...
    if removed {
        ...
        println!("{}", "No orca hook found in settings.".yellow());
    }
...
/// Update orca by re-running the platform installer.
...
fn handle_rollback(target_version: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!("Rolling back orca...");

    match rollback(target_version) {
        Ok(message) => {
            println!("{message}");
            println!("\nRestart orca to use the restored version.");
            Ok(())
        }
...
/// `orca update --system` or `orca update --from-source`
...
fn update_installer_tag(
    ...
    match requested_version {
        ...
        None => match crate::update::check_for_update(true) {
            ...
            Err(err) => Err(format!(
                "Failed to resolve latest release for orca update: {err}. Re-run with --version vX.Y.Z to install a known release."
            )
            .into()),
        },
    }
    ...
    fn create() -> Result<Self, Box<dyn std::error::Error>> {
        ...
        for attempt in 0..100_u32 {
            let nanos = std::time::SystemTime::now()
                ...
                .as_nanos();
            let path = base.join(format!("orca-install-{process_id}-{nanos}-{attempt}"));
...
fn download_verified_installer(
    ...
    if try_download_file(sha_url, &checksum_path)? {
        ...
        eprintln!("orca update: {artifact_name} sha256 verified.");
    } else {
        eprintln!(
            "orca update: {artifact_name}.sha256 not published for this tag; proceeding without verification."
        );
    }
...
fn self_update_unix(update: UpdateCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let script_url = format!(
        "https://raw.githubusercontent.com/christopherkarani/Orca/{normalized_tag}/install.sh"
    );
    ...
    let sha_url = format!(
        "https://github.com/christopherkarani/Orca/releases/download/{normalized_tag}/install.sh.sha256"
    );

    eprintln!("orca update: downloading and verifying install.sh from {normalized_tag}.");
...
fn self_update_windows(update: UpdateCommand) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let normalized_tag = update_installer_tag(requested_version.as_deref())?;
    let script_url = format!(
        "https://raw.githubusercontent.com/christopherkarani/Orca/{normalized_tag}/install.ps1"
    );
    let sha_url = format!(
        "https://github.com/christopherkarani/Orca/releases/download/{normalized_tag}/install.ps1.sha256"
    );

    eprintln!("orca update: downloading and verifying install.ps1 from {normalized_tag}.");
...
/// Path to the user-level Grok orca hook file (`~/.grok/hooks/orca.json`).
///
...
/// Path to a project-level Grok orca hook file (`<repo>/.grok/hooks/orca.json`).
///
/// Returns `Err` if the current directory is not inside a git repository.
fn project_grok_hook_path() -> Result<std::path::PathBuf, Box<dyn std::error::Error>> {
    let repo_root = find_repo_root_from_cwd()
        .ok_or("Not inside a git repository — cannot determine project root")?;
    Ok(repo_root.join(".grok").join("hooks").join("orca.json"))
}
...
/// Check if orca is in PATH
fn which_orca() -> Option<std::path::PathBuf> {
...
// NOTE: These diagnostic types are scaffolding for future `orca doctor` enhancements.
// ============================================================================
...
struct HookDiagnostics {
    ...
    /// Number of orca hook entries found
    orca_hook_count: usize,
    /// Orca hooks found with wrong matcher (not "Bash")
    wrong_matcher_hooks: Vec<String>,
    /// Orca hooks pointing to absolute path that doesn't exist
    missing_executable_hooks: Vec<String>,
    /// Other non-orca hooks in `PreToolUse`
...
fn validate_config_diagnostics() -> ConfigDiagnostics {
    ...
    if let Ok(value) = std::env::var(crate::config::ENV_CONFIG_PATH) {
        let Some(path) = crate::config::resolve_config_path_value(&value, cwd.as_deref()) else {
            diag.parse_error = Some("ORCA_CONFIG is set but empty".to_string());
            return diag;
        };
        if !path.exists() {
            diag.parse_error = Some(format!(
                "ORCA_CONFIG points to a missing file: {}",
                path.display()
            ));
    ...
    // Find default/user/project config path (when ORCA_CONFIG isn't set)
    if diag.config_path.is_none() {
        let cfg_path = config_path();
        if cfg_path.exists() {
            diag.config_path = Some(cfg_path);
        } else if let Some(repo_root) = find_repo_root_from_cwd() {
            let project_config = repo_root.join(".orca.toml");
...
// ============================================================================
// Developer Tools (orca dev)
// ============================================================================

/// Handle all `orca dev` subcommands
...
mod tests {
    ...
    #[test]
    fn uninstall_from_settings_removes_orca_entries() {
        let other = serde_json::json!({
            "matcher": "Bash",
            "hooks": [{ "type": "command", "command": "other-hook" }]
        });
        ...
        let removed = uninstall_orca_hook_from_settings(&mut settings).expect("uninstall ok");
        assert!(removed);

        let pre = settings["hooks"]["PreToolUse"].as_array().unwrap();
        assert_eq!(pre.iter().filter(|e| is_orca_hook_entry(e)).count(), 0);
        assert_eq!(pre.len(), 1, "should retain non-orca hook");
    ...
    fn uninstall_from_settings_errors_on_invalid_pre_tool_use_type() {
        ...
        let err = uninstall_orca_hook_from_settings(&mut settings).expect_err("should error");
        assert!(err.to_string().contains("PreToolUse"));
    }
    ...
    fn test_cli_parse_init() {
        let cli = Cli::parse_from(["orca", "init"]);
        assert!(matches!(cli.command, Some(Command::Init { .. })));
    }

    #[test]
    fn test_cli_parse_init_auto() {
        let cli = Cli::parse_from(["orca", "init", "--auto"]);
    ...
    fn test_cli_parse_init_dry_run() {
        let cli = Cli::parse_from(["orca", "init", "--dry-run"]);
    ...
    fn test_cli_parse_init_auto_with_project_dir() {
        let cli = Cli::parse_from(["orca", "init", "--auto", "--project-dir", "/tmp/myproject"]);
    ...
    fn test_generate_config_with_packs() {
        ...
        assert!(config.contains("orca init --auto"));
    }

    #[test]
    fn test_cli_parse_update() {
        let cli = Cli::parse_from(["orca", "update", "--version", "v0.2.0"]);
    ...
    fn test_cli_parse_update_no_configure() {
        let cli = Cli::parse_from(["orca", "update", "--no-configure"]);
        if let Some(Command::Update(update)) = cli.command {
    ...
    fn test_cli_parse_update_binary_only_alias() {
        let cli = Cli::parse_from(["orca", "update", "--binary-only"]);
        if let Some(Command::Update(update)) = cli.command {
    ...
    fn test_cli_parse_install_project() {
        let cli = Cli::parse_from(["orca", "install", "--project"]);
    ...
    fn test_cli_parse_install_force_and_project() {
        let cli = Cli::parse_from(["orca", "install", "--force", "--project"]);
        if let Some(Command::Install {
            force,
            project,
            grok,
    ...
    fn test_cli_parse_install_grok() {
        let cli = Cli::parse_from(["orca", "install", "--grok"]);
        if let Some(Command::Install {
            force,
            project,
            grok,
    ...
    fn test_cli_parse_install_grok_with_project() {
        let cli = Cli::parse_from(["orca", "install", "--grok", "--project"]);
        if let Some(Command::Install {
            force,
            project,
            grok,
...
"#,
    ...
    fn hook_diagnostics_default_is_not_healthy() {
        ...
        // Default has settings_valid=false, orca_hook_count=0
    ...
    fn hook_diagnostics_unhealthy_missing_executable() {
        let diag = HookDiagnostics {
            ...
            orca_hook_count: 1,
            wrong_matcher_hooks: vec![],
            missing_executable_hooks: vec!["/nonexistent/path/orca".to_string()],
            other_hooks_count: 0,
        };
        assert!(!diag.is_healthy());
        assert!(diag.has_issues());
    ...
    fn diagnose_hook_wiring_from_json_valid_settings() {
        ...
        // Verify the structure is valid and has orca hook
        ...
        assert!(is_orca_hook_entry(&pre_tool_use[0]));
    }

    #[test]
    fn diagnose_hook_wiring_from_json_wrong_matcher() {
        // orca hook with wrong matcher (Write instead of Bash)
        // Note: is_orca_hook_entry requires BOTH Bash matcher AND orca command,
        // so this entry won't be recognized as a orca hook entry.
        ...
        // Entry has orca command but is_orca_hook_entry returns false due to wrong matcher
        assert!(
            !is_orca_hook_entry(entry),
            "should not be orca hook due to wrong matcher"
        );

        // Verify the command is orca
        let cmd = entry["hooks"][0]["command"].as_str().unwrap();
        assert!(is_orca_command(cmd));
    ...
    #[test]
    fn diagnose_hook_wiring_from_json_multiple_orca_hooks() {
        // Multiple orca hooks (duplicates)
        let settings = serde_json::json!({
            "hooks": {
                "PreToolUse": [
                    {
        ...
        let pre_tool_use = settings["hooks"]["PreToolUse"].as_array().unwrap();
        let orca_count = pre_tool_use
            .iter()
            .filter(|e| is_orca_hook_entry(e))
            .count();

        assert_eq!(orca_count, 2, "should detect duplicate orca hooks");
    }

    #[test]
    fn is_orca_command_recognizes_various_forms() {
        assert!(is_orca_command("orca"));
        assert!(is_orca_command("/usr/local/bin/orca"));
        assert!(is_orca_command("/home/user/.cargo/bin/orca"));
        assert!(is_orca_command("~/.local/bin/orca"));

        assert!(!is_orca_command("other-hook"));
        assert!(!is_orca_command(""));
        assert!(!is_orca_command("orca-wrapper"));
    }
