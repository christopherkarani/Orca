...
pub const ENV_PENDING_EXCEPTIONS_PATH: &str = "ORCA_PENDING_EXCEPTIONS_PATH";
/// Environment override for allow-once entries file path.
pub const ENV_ALLOW_ONCE_PATH: &str = "ORCA_ALLOW_ONCE_PATH";
/// Optional HMAC secret for short-code hardening.
/// When set, codes cannot be forged without knowing the secret.
pub const ENV_ALLOW_ONCE_SECRET: &str = "ORCA_ALLOW_ONCE_SECRET";
...
/// Compare two paths for equality, resolving symlinks on both sides.
///
/// On macOS, temp directories live under `/var/folders` which is a symlink
/// to `/private/var/folders`. `std::env::current_dir()` returns the
/// canonical (resolved) path, while user-written config entries often use
/// the un-resolved path. Without this helper, an allow-once entry created
/// in `/var/folders/…` would fail to match when the hook runs from
/// `/private/var/folders/…`.
#[must_use]
fn paths_equal_resolving_symlinks(a: &Path, b: &Path) -> bool {
    if a == b {
        return true;
    }
    if let Ok(a_canon) = a.canonicalize() {
        if let Ok(b_canon) = b.canonicalize() {
...
impl AllowOnceEntry {
    ...
    pub fn matches_scope(&self, cwd: &Path) -> bool {
        ...
        match self.scope_kind {
            AllowOnceScopeKind::Cwd => paths_equal_resolving_symlinks(cwd, scope_path),
            AllowOnceScopeKind::Project => {
                paths_equal_resolving_symlinks(cwd, scope_path)
                    || cwd.starts_with(scope_path)
                    || cwd
                        .canonicalize()
                        .ok()
                        .map(|c| c.starts_with(scope_path))
                        .unwrap_or(false)
                    || scope_path
                        .canonicalize()
                        .ok()
                        .map(|s| cwd.starts_with(&s))
                        .unwrap_or(false)
                    || scope_path
                        .canonicalize()
                        .ok()
                        .and_then(|s| cwd.canonicalize().ok().map(|c| c.starts_with(&s)))
                        .unwrap_or(false)
            }
        }
    }
...
impl PendingExceptionStore {
    ...
    pub fn default_path(cwd: Option<&Path>) -> PathBuf {
        ...
        // Prefer XDG_CONFIG_HOME when set (consistent with config.rs/cli.rs)
        if let Ok(xdg_home) = env::var("XDG_CONFIG_HOME") {
            let path = PathBuf::from(xdg_home)
                .join(CONFIG_DIR)
                .join(PENDING_EXCEPTIONS_FILE);
            return path;
        }

        // Always use XDG-style path when HOME is available (consistent with cli.rs)
        ...
        base.join(CONFIG_DIR).join(PENDING_EXCEPTIONS_FILE)
    }
...
impl AllowOnceStore {
    ...
    pub fn default_path(cwd: Option<&Path>) -> PathBuf {
        ...
        if let Ok(xdg_home) = env::var("XDG_CONFIG_HOME") {
            let path = PathBuf::from(xdg_home)
                .join(CONFIG_DIR)
                .join(ALLOW_ONCE_FILE);
            return path;
        }

        // Always use XDG-style path when HOME is available (consistent with cli.rs)
        if let Some(home) = dirs::home_dir() {
            let path = home.join(".config").join(CONFIG_DIR).join(ALLOW_ONCE_FILE);
            return path;
        }
        ...
        let base = dirs::config_dir()
            .unwrap_or_else(|| dirs::home_dir().unwrap_or_default().join(".config"));
        base.join(CONFIG_DIR).join(ALLOW_ONCE_FILE)
    }
...
///
/// If `ORCA_ALLOW_ONCE_SECRET` is set, uses HMAC-SHA256 for tamper resistance.
