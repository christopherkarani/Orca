...
//!
//! ```ignore
//! use orca_rs::context::{classify_command, SpanKind};
//!
                ...
                TokenizerState::DoubleQuote => {
                    match byte {
                        ...
                        b'$' if i + 1 < len && bytes[i + 1] == b'(' => {
                            ...
                            // The span logic in orca expects flat spans.
...
pub fn sanitize_for_pattern_matching(command: &str) -> Cow<'_, str> {
    ...
    for (i, token) in tokens.iter().enumerate() {
        ...
        if segment_cmd_is_all_args_data {
            ...
            // `2>/etc/passwd`, `"x">/etc/passwd`). The orca tokenizer
...
/// args of the form `data>/etc/passwd` where the orca tokenizer keeps
