...
//!
//! When ORCA blocks a command, users need actionable guidance:
...
pub enum SuggestionKind {
    ...
    /// e.g., "To allow: `orca allow core.git:reset-hard --reason '...'`"
    AllowSafely,
}
...
///
/// ```
/// use orca_rs::suggestions::get_suggestions;
///
...
/// - Points to `orca explain` for details
///
/// # Arguments
///
        ...
        (Some(pack), Some(pattern)) => {
            format!(
                "This command matched the destructive pattern `{pack}:{pattern}`. \
                 Run `orca explain` on this command for details and safer alternatives."
            )
        }
        (Some(pack), None) => {
            format!(
...
///
/// ```
/// use orca_rs::suggestions::get_explanation;
///
