//! Repository scanning (`orca scan`) for destructive commands.
//!
...
//! # Output schema (v1)
//!
//! `orca scan --format json` emits a `ScanReport` containing:
...
///
/// Loaded from `.orca/hooks.toml` (if present).
...
/// Parse `.orca/hooks.toml` and return (typed config, warnings).
///
...
/// matters; the scan path is offline (`orca scan .` runs once, deliberately,
...
///
/// ```ignore
/// use orca_rs::scan::scan_paths_with_progress;
///
...
"#;
    ...
    fn scan_paths_records_path_not_found_for_missing_target() {
        ...
        let target =
            std::env::temp_dir().join(format!("orca-scan-missing-{}-{}", std::process::id(), 42));
