...
//!
//! This module provides functionality to check for newer versions of Orca-rs
...
use crate::branding::{CLI_NAME, CONFIG_DIR, GITHUB_OWNER, GITHUB_REPO};
...
pub struct BackupEntry {
    ...
    /// Original path where orca was installed.
...
pub fn backup_dir() -> Option<PathBuf> {
    dirs::data_dir().map(|d| d.join(CONFIG_DIR).join("backups"))
}
...
fn backup_artifact_name(entry: &BackupEntry) -> String {
    entry
        ...
        .map(str::to_string)
        .unwrap_or_else(|| format!("{CLI_NAME}-{}-{}", entry.version, entry.created_at))
}

fn generate_unique_backup_name(dir: &Path, version: &str, timestamp: u64) -> String {
    let base = format!("{CLI_NAME}-{version}-{timestamp}");
...
pub fn format_backup_list(backups: &[BackupEntry], use_color: bool) -> String {
    ...
    if backups.is_empty() {
        return if use_color {
            "\x1b[33mNo backup versions available.\x1b[0m\n\
             Run 'orca update' to create a backup of the current version."
                .to_string()
        } else {
    ...
    if use_color {
        writeln!(
            output,
            "Use '\x1b[1morca update --rollback\x1b[0m' to restore the most recent backup"
        )
        .ok();
        writeln!(
            output,
            "Use '\x1b[1morca update --rollback <version>\x1b[0m' to restore a specific version"
        )
        .ok();
    } else {
        writeln!(
            output,
            "Use 'orca update --rollback' to restore the most recent backup"
        )
        .ok();
        writeln!(
            output,
            "Use 'orca update --rollback <version>' to restore a specific version"
        )
        .ok();
    }
...
fn cache_path() -> Option<PathBuf> {
    dirs::cache_dir().map(|d| d.join(CONFIG_DIR).join("version_check.json"))
}
...
/// Get the current version of orca from Cargo.toml.
#[must_use]
...
pub fn format_check_result(result: &VersionCheckResult, use_color: bool) -> String {
    ...
    if use_color {
        ...
        if result.update_available {
            writeln!(
                output,
                "\x1b[33m✨ Update available!\x1b[0m Run '\x1b[1morca update\x1b[0m' to upgrade"
            )
            .ok();
        } else {
        ...
        if result.update_available {
            writeln!(output, "Update available! Run 'orca update' to upgrade").ok();
        } else {
...
pub fn get_update_notice(use_color: bool) -> Option<String> {
    ...
    let notice = if use_color {
        format!(
            "\x1b[33m!\x1b[0m A new version of orca is available: {} -> {}\n  Run '\x1b[1morca update\x1b[0m' to upgrade",
...
{
    get_env("ORCA_NO_UPDATE_CHECK").is_none_or(|value| !disable_env_flag_enabled(&value))
}
...
mod tests {
    ...
    fn test_release_url_for_tag_uses_exact_tag() {
        assert_eq!(
            ...
            "https://github.com/christopherkarani/Orca/releases/tag/v2.1.0"
        );
        assert_eq!(
            release_url_for_tag("2.1.0"),
            "https://github.com/christopherkarani/Orca/releases/tag/2.1.0"
        );
    }
    ...
    fn test_release_url_for_tag_empty_uses_latest() {
        assert_eq!(
            release_url_for_tag(""),
            "https://github.com/christopherkarani/Orca/releases/latest"
        );
    }
    ...
    fn test_generate_unique_backup_name_avoids_collision() {
        ...
        let base = format!("{CLI_NAME}-0.2.12-1737200000");
    ...
    fn test_backup_artifact_name_prefers_metadata() {
        let with_name = BackupEntry {
            ...
            artifact_name: Some("0.2.12-1737200000-123456789".to_string()),
            original_path: std::path::PathBuf::from("/usr/local/bin/orca"),
        };
        assert_eq!(
            backup_artifact_name(&with_name),
            "0.2.12-1737200000-123456789"
        );
        ...
        assert_eq!(
            backup_artifact_name(&legacy),
            format!("{CLI_NAME}-0.2.11-1737100000")
        );
    }

    #[test]
    fn test_backup_artifact_name_rejects_path_like_metadata() {
        let fallback = format!("{CLI_NAME}-0.2.12-1737200000");
    ...
    fn test_backup_names_to_prune_still_removes_non_preserved_overflow() {
        ...
        let rollback_prune = backup_names_to_prune(&backups, Some(&rollback_target));
        assert_eq!(rollback_prune, vec![format!("{CLI_NAME}-0.2.11-300")]);
    ...
    fn test_backup_names_to_prune_keeps_cap_when_preserved_target_is_recent() {
        ...
        let rollback_prune = backup_names_to_prune(&backups, Some(&rollback_target));
        assert_eq!(rollback_prune, vec![format!("{CLI_NAME}-0.2.10-200")]);
        assert!(!rollback_prune.contains(&rollback_target));
    }
    ...
    fn test_format_check_result_update_available() {
        ...
        assert!(output.contains("orca update"));
    }
    ...
    fn test_backup_entry_serialization() {
        let entry = BackupEntry {
            version: "0.2.12".to_string(),
            created_at: 1_737_200_000,
            artifact_name: Some("0.2.12-1737200000".to_string()),
            original_path: std::path::PathBuf::from("/usr/local/bin/orca"),
        };
    ...
    fn test_backup_dir_exists() {
        ...
        let dir = backup_dir();
        assert!(dir.is_some());
        // The path should contain "orca" and "backups"
        let path = dir.unwrap();
    ...
    fn test_is_update_check_enabled_empty_value() {
        ...
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_NO_UPDATE_CHECK", "")]);
        assert!(is_update_check_enabled_with(|key| {
            env_map.get(key).map(|v| (*v).to_string())
        }));
    }
    ...
    fn test_is_update_check_disabled_various_values() {
        for val in &["1", "true", "yes", "anything"] {
            let env_map: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::from([("ORCA_NO_UPDATE_CHECK", *val)]);
            assert!(
                !is_update_check_enabled_with(|key| { env_map.get(key).map(|v| (*v).to_string()) }),
                "ORCA_NO_UPDATE_CHECK={val} should disable update check"
            );
        }
    }
    ...
    fn test_is_update_check_falsey_values_do_not_disable() {
        for val in &["0", "false", "no", "n", "off", " FALSE "] {
            ...
            assert!(
                is_update_check_enabled_with(|key| { env_map.get(key).map(|v| (*v).to_string()) }),
                "ORCA_NO_UPDATE_CHECK={val} should not disable update check"
            );
        }
    }
}
