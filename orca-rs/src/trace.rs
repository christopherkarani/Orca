//! Explain trace data model for `orca explain` mode.
//!
...
//!
//! ```ignore
//! use orca_rs::trace::TraceCollector;
//! use orca_rs::evaluator::EvaluationDecision;
//!
...
/// JSON schema version for `orca explain --format json`.
...
///
/// Contains all information needed for `orca explain` output formatting.
#[derive(Debug, Clone)]
...
impl ExplainTrace {
    ...
    pub fn format_pretty(&self, use_color: bool) -> String {
        ...
        out.push_str(&format!("{bold}ORCA EXPLAIN{reset}\n"));
        out.push_str(&format!(
            "{bold}══════════════════════════════════════════════════════════════════{reset}\n\n"
        ));
...
/// Top-level JSON output structure for `orca explain --format json`.
...
mod tests {
    ...
    fn format_pretty_allow_simple() {
        ...
        assert!(pretty.contains("ORCA EXPLAIN"));
