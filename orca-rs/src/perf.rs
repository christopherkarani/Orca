//! Performance budgets for orca.
//!
//! This module defines explicit latency budgets for all orca operations.
...
//! # Budget Philosophy
//!
//! orca runs on every Bash command, so performance is critical. We define:
...
//! This ensures orca never blocks a user's workflow indefinitely.
...
impl Deadline {
    ...
    pub fn remaining(&self) -> Option<Duration> {
        ...
        Some(self.max_duration.checked_sub(elapsed).unwrap())
    }
...
/// in config or `ORCA_HOOK_TIMEOUT_MS=0` env var. A zero or near-zero timeout
