//! Command history database for Orca-rs.
//!
//! This module provides SQLite-based history collection and querying for
//! tracking all commands evaluated by Orca-rs across agent sessions.
//!
//! # Architecture
//!
//! ```text
...
//!
//! ```ignore
//! use orca_rs::history::{HistoryDb, CommandEntry, Outcome};
//!
...
pub const ENV_HISTORY_DB_PATH: &str = "ORCA_HISTORY_DB";

/// Environment variable to disable history collection entirely.
pub const ENV_HISTORY_DISABLED: &str = "ORCA_HISTORY_DISABLED";
...
impl HistoryWriter {
    ...
    pub fn new(db_path: Option<std::path::PathBuf>, config: &HistoryConfig) -> Self {
        ...
        let handle = match thread::Builder::new()
            .name("orca-history-writer".to_string())
            .spawn(move || {
