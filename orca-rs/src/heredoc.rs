...
///
/// ```ignore
/// use orca_rs::heredoc::{check_triggers, TriggerResult};
///
...
impl ScriptLanguage {
    ...
    pub fn from_command(cmd: &str) -> Self {
        ...
        if matches_interpreter("python") {
            ...
            // re-evaluation surfaces the same rules. This is what lets orca
...
///
/// Used by `orca explain` to show why a particular language was detected.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
...
///
/// ```ignore
/// use orca_rs::heredoc::{extract_content, ExtractionLimits, ExtractionResult};
///
...
mod tests {
    ...
    mod tier2_extraction {
        ...
        /// `powershell.exe -Command '<inner>'`. orca must descend into the
