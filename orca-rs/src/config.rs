//! Configuration system for Orca-rs.
//!
//! Supports layered configuration from multiple sources:
//! 1. Environment variables (highest priority)
//! 2. Project config (`.orca.toml` in repo root)
//! 3. User config (`$XDG_CONFIG_HOME/orca/config.toml`, `~/.config/orca/config.toml`, or
//!    platform-native config dir)
//! 4. System config (`/etc/orca/config.toml`)
//! 5. Compiled defaults (lowest priority)

use crate::branding::{CONFIG_DIR, ENV_PREFIX, PROJECT_CONFIG_FILE};
...
/// Maximum config file size Orca-rs is willing to read into memory.
///
...
pub(crate) const ENV_CONFIG_PATH: &str = "ORCA_CONFIG";
...
///
/// `session_*` thresholds count occurrences in the current orca process via
...
/// callers like `orca test`, the MCP server, or repeated CLI evaluations.
///
...
pub struct GeneralConfig {
    ...
    /// When enabled, orca will spawn a background thread to check for updates
    /// and show a notice if a newer version is available.
    /// Default: true. Disable with truthy `ORCA_NO_UPDATE_CHECK`
    /// or `check_updates` = false.
    pub check_updates: bool,

    /// Whether to self-heal the hook registration in settings.json.
    /// When enabled, every hook invocation checks that the orca entry is still
    ...
    /// mid-session.
    /// Default: true. Disable with `ORCA_NO_SELF_HEAL` or `self_heal_hook = false`.
...
pub struct PacksConfig {
    ...
    /// - `~/.config/orca/packs/*.yaml` - User-level packs
    /// - `.orca/packs/*.yaml` - Project-level packs
    /// - `/etc/orca/packs/*.yaml` - System-wide packs
    ///
...
pub struct PolicyConfig {
    ...
    /// - `default_mode` is ignored and orca reverts to severity-based defaults.
    ///
    /// Accepted formats:
...
/// Settings for layered allowlist files (`.orca/allowlist.toml`,
/// `~/.config/orca/allowlist.toml`, and `/etc/orca/allowlist.toml`).
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct AllowlistConfig {
    ...
    /// user explicitly opts in or runs `orca allowlist prune`.
...
pub struct GitAwarenessConfig {
    ...
    /// if orca cannot detect a git repository. The command will still be
...
///
/// Defines how orca should behave when invoked by a specific AI coding agent.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
#[serde(default)]
        ...
        #[cfg(debug_assertions)]
        {
            ...
            for rule in &rules {
                if !seen.insert(&rule.pattern) {
                    eprintln!(
                        "orca: warning: duplicate allowlist pattern: {}",
                        rule.pattern
                    );
                }
            }
...
impl Config {
    ...
    /// 4. User config (`$XDG_CONFIG_HOME/orca/config.toml`, `~/.config/orca/config.toml`,
    ...
    ///
    /// The `/etc/orca/config.toml` path is treated as a privileged config
    ...
    ///
    /// Checks XDG_CONFIG_HOME, XDG-style (`~/.config/orca/`), and platform-native paths.
    /// This ensures users can use `~/.config/orca/config.toml` on all platforms,
    ...
    fn load_user_config_layer() -> Option<ConfigLayer> {
        ...
        // Next try XDG-style path (~/.config/orca/config.toml)
    ...
    {
        // ORCA_PACKS="core,database.postgresql,kubernetes"
        ...
        // ORCA_CUSTOM_PATHS="/path/to/pack.yaml,~/.config/orca/packs/*.yaml"
        ...
        // ORCA_CHECK_UPDATES=true|false|1|0
        ...
        // ORCA_NO_UPDATE_CHECK=1 (override)
        if let Some(disable) = get_env(&format!("{ENV_PREFIX}_NO_UPDATE_CHECK")) {
            if env_disable_flag_enabled(&disable) {
                self.general.check_updates = false;
            }
        }

        // ORCA_SELF_HEAL_HOOK=true|false|1|0
        ...
        // ORCA_NO_SELF_HEAL=1 (override)
        if let Some(disable) = get_env(&format!("{ENV_PREFIX}_NO_SELF_HEAL")) {
            if env_disable_flag_enabled(&disable) {
                self.general.self_heal_hook = false;
            }
        }

        // ORCA_HOOK_TIMEOUT_MS=200
        ...
        // ORCA_HIGH_CONTRAST=1
        if let Some(high_contrast) = get_env(&format!("{ENV_PREFIX}_HIGH_CONTRAST")) {
        ...
        // ORCA_HEREDOC_ENABLED=true|false|1|0
        ...
        // ORCA_HEREDOC_LANGUAGES=python,bash,javascript
        ...
        // ORCA_POLICY_DEFAULT_MODE=deny|warn|log
        ...
        // ORCA_POLICY_OBSERVE_UNTIL=2030-01-01T00:00:00Z
        ...
        // ORCA_HISTORY_ENABLED=true|false|1|0
        ...
        // ORCA_HISTORY_REDACTION_MODE=none|pattern|full
        ...
        // ORCA_INTERACTIVE_ENABLED=true|false|1|0
        ...
        // ORCA_INTERACTIVE_VERIFICATION=code|command|none
        ...
        // ORCA_INTERACTIVE_TIMEOUT_SECONDS=5
        ...
        // ORCA_INTERACTIVE_CODE_LENGTH=4
        ...
        // ORCA_INTERACTIVE_MAX_ATTEMPTS=3
        ...
        // ORCA_INTERACTIVE_ALLOW_NON_TTY_FALLBACK=true|false|1|0
        ...
        // ORCA_INTERACTIVE_DISABLE_IN_CI=true|false|1|0
        ...
        // ORCA_INTERACTIVE_REQUIRE_ENV=ORCA_INTERACTIVE
        ...
        // ORCA_GIT_AWARENESS_ENABLED=true|false|1|0
        ...
        // ORCA_GIT_PROTECTED_BRANCHES=main,production,release/*
        ...
        // ORCA_GIT_PROTECTED_STRICTNESS=critical|high|medium|all
        ...
        // ORCA_GIT_RELAXED_BRANCHES=feature/*,experiment/*
        ...
        // ORCA_GIT_RELAXED_STRICTNESS=critical|high|medium|all
        ...
        // ORCA_GIT_DEFAULT_STRICTNESS=critical|high|medium|all
        ...
        // ORCA_GIT_DETACHED_HEAD_STRICTNESS=critical|high|medium|all
        ...
        // ORCA_GIT_RELAXED_DISABLED_PACKS=containers.docker,cloud.aws
        ...
        // ORCA_GIT_SHOW_BRANCH_IN_OUTPUT=true|false|1|0
        ...
        // ORCA_GIT_AWARENESS_WARN_IF_NOT_GIT=true|false|1|0
...
# log_file = "~/.local/share/orca/blocked.log"
...
custom_paths = [
    ...
    # "${repo_root}/.orca/packs/*.yaml",     # Repo-local packs (auto-discovered)
    # "/etc/orca/packs/*.yaml",              # System-wide packs
]
...
#
# If unset, orca uses severity defaults:
# - critical/high => deny
# - medium => warn
# - low => log
#
...
# database_path = "~/.config/orca/history.db"
...
retention_days = 30
max_size_mb = 250
database_path = "/tmp/orca-history.db"
"#;
    ...
    fn test_history_env_overrides() {
        let env_map: std::collections::HashMap<&str, &str> = std::collections::HashMap::from([
            ("ORCA_HISTORY_ENABLED", "true"),
            ("ORCA_HISTORY_REDACTION_MODE", "full"),
        ]);
        let mut config = Config::default();
    ...
    fn test_history_database_path_expansion() {
        ...
        let config = HistoryConfig {
            database_path: Some("~/.config/orca/history.db".to_string()),
            ..Default::default()
        };
        let expanded = config
        ...
        assert!(expanded.to_string_lossy().contains(CONFIG_DIR));
    }
...
"#;
    ...
    fn test_env_high_contrast_override() {
        ...
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_HIGH_CONTRAST", "1")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
...
",
    ...
    fn test_interactive_env_overrides() {
        let env_map: std::collections::HashMap<&str, &str> = std::collections::HashMap::from([
            ("ORCA_INTERACTIVE_ENABLED", "true"),
            ("ORCA_INTERACTIVE_VERIFICATION", "none"),
            ("ORCA_INTERACTIVE_TIMEOUT_SECONDS", "11"),
            ("ORCA_INTERACTIVE_CODE_LENGTH", "6"),
            ("ORCA_INTERACTIVE_MAX_ATTEMPTS", "8"),
            ("ORCA_INTERACTIVE_ALLOW_NON_TTY_FALLBACK", "false"),
            ("ORCA_INTERACTIVE_DISABLE_IN_CI", "false"),
            ("ORCA_INTERACTIVE_REQUIRE_ENV", "ORCA_INTERACTIVE"),
        ]);
        let mut config = Config::default();
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
    ...
    fn test_interactive_env_empty_require_env_clears_requirement() {
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_INTERACTIVE_REQUIRE_ENV", "   ")]);
        let mut config = Config::default();
        config.interactive.require_env = Some("KEEP_ME".to_string());
...
",
    ...
    fn test_git_awareness_env_overrides() {
        let env_map: std::collections::HashMap<&str, &str> = std::collections::HashMap::from([
            ("ORCA_GIT_AWARENESS_ENABLED", "true"),
            ("ORCA_GIT_PROTECTED_BRANCHES", "main, production, release/*"),
            ("ORCA_GIT_PROTECTED_STRICTNESS", "all"),
            ("ORCA_GIT_RELAXED_BRANCHES", "feature/*, sandbox/*"),
            ("ORCA_GIT_RELAXED_STRICTNESS", "critical"),
            ("ORCA_GIT_DEFAULT_STRICTNESS", "medium"),
            (
                "ORCA_GIT_RELAXED_DISABLED_PACKS",
                "containers.docker, cloud.aws",
            ),
            ("ORCA_GIT_SHOW_BRANCH_IN_OUTPUT", "false"),
            ("ORCA_GIT_AWARENESS_WARN_IF_NOT_GIT", "true"),
        ]);
        let mut config = Config::default();
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
    ...
    fn test_heredoc_env_overrides_enabled_timeout_languages() {
        let env_map: std::collections::HashMap<&str, &str> = std::collections::HashMap::from([
            ("ORCA_HEREDOC_ENABLED", "0"),
            ("ORCA_HEREDOC_TIMEOUT_MS", "123"),
            ("ORCA_HEREDOC_LANGUAGES", "python, bash, js, unknown_value"),
        ]);
        let mut config = Config::default();
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
    ...
    fn test_env_override_verbose_numeric() {
        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_VERBOSE", "0")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        assert!(!config.general.verbose);

        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_VERBOSE", "2")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        assert!(config.general.verbose);
    }
    ...
    fn test_env_override_check_updates() {
        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_CHECK_UPDATES", "0")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        assert!(!config.general.check_updates);

        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_NO_UPDATE_CHECK", "1")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        assert!(!config.general.check_updates);

        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_NO_UPDATE_CHECK", "false")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        assert!(config.general.check_updates);

        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_NO_UPDATE_CHECK", "0")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        assert!(config.general.check_updates);
    }
    ...
    fn test_env_override_no_self_heal_falsey_values_do_not_disable() {
        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_NO_SELF_HEAL", "1")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
        ...
        for value in ["0", "false", "no", "n", "off", ""] {
            let mut config = Config::default();
            let env_map: std::collections::HashMap<&str, &str> =
                std::collections::HashMap::from([("ORCA_NO_SELF_HEAL", value)]);
            config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
            assert!(
                config.general.self_heal_hook,
                "ORCA_NO_SELF_HEAL={value:?} should not disable self-heal"
            );
        }
    }
    ...
    fn test_env_override_hook_timeout_ms() {
        let mut config = Config::default();
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_HOOK_TIMEOUT_MS", "150")]);
        config.apply_env_overrides_from(|key| env_map.get(key).map(|v| (*v).to_string()));
    ...
    fn test_policy_env_override_default_mode() {
        let env_map: std::collections::HashMap<&str, &str> =
            std::collections::HashMap::from([("ORCA_POLICY_DEFAULT_MODE", "warn")]);
...
"#;
    ...
    fn test_expand_custom_paths_repo_root_token() {
        ...
        let packs = PacksConfig {
            custom_paths: vec!["${repo_root}/.orca/packs/*.yaml".to_string()],
            ..PacksConfig::default()
        };
    ...
    fn test_expand_custom_paths_repo_root_no_repo() {
        ...
        let packs = PacksConfig {
            custom_paths: vec![
                "${repo_root}/.orca/packs/*.yaml".to_string(),
    ...
    fn read_config_file_bounded_returns_none_for_missing_path() {
        let path =
            std::env::temp_dir().join(format!("orca-config-missing-{}-{}", std::process::id(), 42));
        let read = read_config_file_bounded(&path, ConfigSource::Untrusted);
        assert!(read.is_none());
