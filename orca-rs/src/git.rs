...
//!
//! ```ignore
//! use orca_rs::git::get_current_branch;
//!
...
mod tests {
    ...
    fn test_head_file_parsing_sha256_hash_is_detached_not_not_git_repo() {
        ...
        // disabled branch-strictness policy across every orca invocation
    ...
    fn test_get_current_branch_returns_some_in_git_repo() {
        // This test runs in the orca repo, so should return a branch
    ...
    fn test_is_in_git_repo() {
        ...
        assert!(
            is_in_git_repo_at_path(temp.path()),
            "initialized temp repo should be detected as a git repo"
        );
    }
