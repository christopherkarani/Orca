# orca scan: Pre-Commit Integration Guide

> Protect your repositories from destructive commands **before** they're committed.

This guide covers how to integrate `orca scan` into your pre-commit workflow safely, with a focus on gradual rollout and minimizing false positive friction.

---

## What orca scan Is (and Is Not)
...
The key difference from the real-time hook (`orca` on its own):

| Mode | Protects | When |
|------|----------|------|
| **Hook** (`orca`) | Interactive agent commands | At execution time |
| **Scan** (`orca scan`) | Commands in committed files | At commit/CI time |
...
This creates `.git/hooks/pre-commit` with a orca-managed hook that runs `orca scan --staged` before each commit.
...
- Use `orca explain "<command>"` to understand why something was flagged
...
2. **Consider enabling warning-as-fail** for specific high-risk patterns:
   ...
   orca scan --staged --fail-on warning
   ```
...
```yaml
# .github/workflows/orca-scan.yml
name: ORCA Scan
on: [pull_request]
jobs:
  scan:
    ...
    steps:
      ...
      - run: orca scan --git-diff origin/${{ github.base_ref }}...HEAD --fail-on error
```
...
[ERROR] .github/workflows/ci.yml:42
  ...
  → orca allow core.git:reset-hard -r "CI cleanup" --project
```
...
orca allow core.git:reset-hard -r "Used for CI cleanup after tests" --project

# Or allowlist a specific command (exact match)
orca allowlist add-command "git reset --hard HEAD" -r "CI cleanup" --project
```

**Important safety notes:**

1. **Always provide a reason** (`-r "..."`) - document why this is safe
2. **Prefer `--project`** for project-specific overrides (stored in `.orca/allowlist.toml`)
3. **Use expiration** for temporary overrides:
   ```bash
   orca allow core.git:reset-hard -r "Migration" --expires "2026-02-01T00:00:00Z" --project
   ```
...
CLI flags always take precedence over `.orca/hooks.toml`.
...
Create with: `npx husky add .husky/pre-commit "orca scan --staged"`
...
### "orca not found in PATH"

The pre-commit hook cannot find `orca`. Either:

1. Install orca globally: `cargo install orca_rs`
2. Add orca's location to PATH before running git commands
3. Use an absolute path in your hook configuration

### "Refusing to overwrite existing pre-commit hook"

You already have a pre-commit hook not installed by orca. Options:

1. **Add orca to your existing hook**: Add `orca scan --staged` to your hook script
2. **Replace the hook**: Delete it manually, then re-run `orca scan install-pre-commit`

### False positives

If orca flags a command that's safe:

1. **Investigate**: `orca explain "the-command"` to understand why
2. **Allowlist if needed**: `orca allow <rule_id> -r "reason" --project`
...
1. **Install**: `orca scan install-pre-commit`
2. **Configure**: Create `.orca/hooks.toml` with your settings
3. **Start conservative**: `fail_on = "error"` initially
4. **Expand gradually**: Add more file types, consider warning-as-fail
5. **Allowlist false positives**: `orca allow <rule_id> -r "reason" --project`
