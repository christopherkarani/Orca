...
orca currently blocks destructive commands via regex pattern matching on the raw command string. However, attackers or accidental misuse can embed destructive code inside heredocs, here-strings, or inline interpreter flags (`python -c`, `bash -c`), bypassing pattern detection.
...
1. **Performance**: orca runs on EVERY bash command. Total latency budget: <10ms typical, <50ms worst-case
...
Rationale: A hung or crashed hook is worse than a missed detection. Diagnostics enable `orca explain` to surface issues for review.
...
- Diagnostics should be emitted with enough context for `orca explain` or logs.
