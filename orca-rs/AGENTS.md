...
**Why this matters:** The `orca update` command and install URLs historically referenced `master`. If `master` falls behind `main`, users get stale code. We had a bug where `master` was **497 commits behind**, causing users to see old installer behavior.
...
## Orca-rs — This Project

**Crate:** `orca_rs` · **Binary:** `orca`

**This is the project you're working on.** Orca-rs is a high-performance Claude Code hook that blocks destructive commands before they execute. It protects against dangerous git commands, filesystem operations, database queries, container commands, and more through a modular pack system.
...
| `src/error_codes.rs` | Standardized ORCA-XXXX error codes |
...
<!-- orca-machine-readable-v1 -->

## ORCA Hook Protocol (Machine-Readable Reference)

> This section provides structured documentation for AI agents integrating with orca.

### JSON Input Format

orca reads from stdin in Claude Code's `PreToolUse` hook format:
...
**Required fields:**
- `tool_name`: Must be `"Bash"` for orca to process (other tools are ignored)
- `tool_input.command`: The shell command string to evaluate

### JSON Output Format (Denial)

When a command is blocked, orca outputs JSON to stdout:
...
```json
{
  "hookSpecificOutput": {
    ...
    "remediation": {
      ...
      "allowOnceCommand": "orca allow-once a1b2c3"
    }
  }
}
```
...
| `allowOnceCode` | `string` | Short code for `orca allow-once` |
...
```bash
output=$(echo "$hook_input" | orca 2>/dev/null)
if [ -z "$output" ]; then
  echo "ALLOWED"
else
  echo "DENIED: $output"
...
ORCA uses standardized error codes in the format `ORCA-XXXX` for machine-parseable error handling.

### Error Categories

| Range | Category | Description |
|-------|----------|-------------|
| ORCA-1xxx | `pattern_match` | Pattern matching and evaluation errors |
| ORCA-2xxx | `configuration` | Configuration loading and parsing errors |
| ORCA-3xxx | `runtime` | Runtime and execution errors |
| ORCA-4xxx | `external` | External integration errors |

### Common Error Codes

| Code | Description | Typical Cause |
|------|-------------|---------------|
| `ORCA-1001` | Pattern compilation failed | Invalid regex syntax in pattern |
| `ORCA-1002` | Pattern match timeout | Complex pattern taking too long |
| `ORCA-2001` | Config file not found | Missing configuration file |
| `ORCA-2002` | Config parse error | Invalid TOML/JSON syntax |
| `ORCA-2004` | Allowlist load error | Invalid allowlist file |
| `ORCA-3001` | JSON parse error | Malformed JSON input |
| `ORCA-3002` | IO error | File read/write failure |
| `ORCA-4001` | External pack load failed | Invalid external pack YAML |
...
```bash
orca allowlist add <ruleId> --project
# Example: orca allowlist add core.git:reset-hard --project
```

Allowlist files (in priority order):
1. `.orca/allowlist.toml` (project)
2. `~/.config/orca/allowlist.toml` (user)
3. `/etc/orca/allowlist.toml` (system)
...
List all packs: `orca packs --verbose`
...
| Command | Purpose |
|---------|---------|
| `orca explain "<command>"` | Detailed trace of why command is blocked/allowed |
| `orca allow-once <code>` | Allow a blocked command for 24 hours |
| `orca allowlist add <ruleId> --project` | Permanently allow a rule |
| `orca packs` | List enabled packs |
| `orca packs --verbose` | List all packs with pattern counts |
| `orca scan .` | Scan codebase for destructive patterns |
| `orca --version` | Show version and build info |

---

## Agent Integration Checklist

When integrating with orca, ensure your agent:
...
- [ ] Uses `orca explain` before asking users to bypass

---

## JSON Schema Reference

Formal JSON Schema definitions (Draft 2020-12) for all orca output formats are available in `docs/json-schema/`:

| Schema | Purpose |
|--------|---------|
| [`hook-output.json`](docs/json-schema/hook-output.json) | PreToolUse hook denial response format |
| [`scan-results.json`](docs/json-schema/scan-results.json) | `orca scan` command output format |
| [`stats-output.json`](docs/json-schema/stats-output.json) | `orca stats` command output format |
| [`error.json`](docs/json-schema/error.json) | Error response formats for various commands |

Use these schemas for:
- Validating orca output in automated pipelines
- Generating type-safe client code
- Understanding the complete output contract

<!-- end-orca-machine-readable -->
...
Coverage is uploaded to Codecov for trend tracking. Dashboard: https://codecov.io/gh/christopherkarani/Orca
...
- `orca-{target}.tar.xz` - Binary archive
- `orca-{target}.tar.xz.sha256` - Checksum
- `orca-{target}.tar.xz.sigstore.json` - Sigstore signature bundle
