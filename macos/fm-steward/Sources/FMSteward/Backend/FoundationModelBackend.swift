...
///
/// - Important: `StewardSession` races residual work (few-shot retrieve then this
///   call) against `timeoutMs` using a task group and cancels the losing child.
///   Implementations **must** honor task cancellation promptly (poll
///   `Task.checkCancellation()` / use cancelable awaits). Structured concurrency
///   joins children before `classify` returns to the caller — a cancellation-blind
///   hang blocks the session past the timeout even when the logical outcome is
///   already `timed_out`. See `CancellationBlindBackend` (tests) for a double that
///   documents this residual; do **not** claim hard wall-clock kill of
///   non-cooperative on-device FM generation.
