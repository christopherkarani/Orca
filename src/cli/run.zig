...
pub fn command(argv: []const []const u8, stdout: anytype, stderr: anytype) !u8 {
    return commandWithStdio(argv, stdout, stderr, .inherit);
}

fn commandWithStdio(argv: []const []const u8, stdout: anytype, stderr: anytype, stdio: core.supervisor.StdioBehavior) !u8 {
...
test "run reports missing command usefully" {
    ...
    const code = try command(&.{ "--", "aegis-definitely-missing-command" }, stdout_stream.writer(), stderr_stream.writer());
...
pub fn commandForTest(argv: []const []const u8, stdout: anytype, stderr: anytype, stdio: core.supervisor.StdioBehavior) !u8 {
    return commandWithStdio(argv, stdout, stderr, stdio);
}
