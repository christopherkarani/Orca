...
pub fn command(argv: []const []const u8, stdout: anytype, stderr: anytype) !u8 {
    ...
    const session_id = intercept.files.resolveSessionId(allocator, workspace_root, options.session) catch |err| {
        try stderr.print("aegis discard: failed to resolve session '{s}': {s}\n", .{ options.session, @errorName(err) });
    ...
    const result = intercept.files.discardStaged(allocator, workspace_root, session_id, options.file, audit_context) catch |err| {
        try stderr.print("aegis discard: failed to discard staged files: {s}\n", .{@errorName(err)});
        return exit_codes.general;
    };
...
fn parseOptions(argv: []const []const u8, stdout: anytype, stderr: anytype) !Options {
    ...
    while (index < argv.len) : (index += 1) {
        ...
        if (std.mem.eql(u8, arg, "--help") or std.mem.eql(u8, arg, "-h")) {
            ...
            index += 1;
            if (index >= argv.len) {
                try stderr.writeAll("aegis discard: --session requires an id or 'last'.\n");
                return error.Usage;
            }
            ...
            index += 1;
            if (index >= argv.len) {
                try stderr.writeAll("aegis discard: --file requires a workspace path.\n");
                return error.Usage;
            }
