...
pub const commands = [_]CommandInfo{
    ...
    .{ .name = "plugin", .summary = "Plugin management and diagnostics", .usage = "orca plugin <doctor|manifest|install|mcp-server> [options]", .details = &.{
        "Subcommands:",
        "  orca plugin doctor [codex|claude|opencode] [--json]",
        "  orca plugin manifest [codex|claude|opencode|all] [--json]",
        "  orca plugin install [codex|claude|opencode|all] [--dry-run] [--path <path>] [--yes]",
    ...
    .{ .name = "hook", .summary = "Host-specific hook adapter for Codex, Claude Code, and OpenCode", .usage = "orca hook <codex|claude|opencode> <event> [--ci]", .details = &.{
