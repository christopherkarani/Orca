...
pub const commands = [_]CommandInfo{
    ...
    .{ .name = "decide", .summary = "Evaluate a policy decision for host plugins", .usage = "aegis decide <command|file|prompt|tool> --json <payload>|--stdin [--ci]", .details = &.{
    ...
    .{ .name = "hook", .summary = "Host-specific hook adapter for Codex and Claude Code", .usage = "aegis hook <codex|claude> <event> [--ci]", .details = &.{
        "Reads a JSON payload from stdin, normalizes host-specific events to Aegis decisions,",
