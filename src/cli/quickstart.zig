...
pub fn command(cwd: std.fs.Dir, argv: []const []const u8, stdout: anytype, stderr: anytype) !u8 {
    ...
    if (!onboarding.policyExists(workspace_root)) {
        try stdout.writeAll("-> Step 2: Creating your first policy...\n");
        const init_code = try onboarding.ensurePolicy(cwd, workspace_root, flags.preset, stdout, stderr, .{
            .missing = "",
            .exists = null,
        });
    ...
    const setup_argv = if (flags.auto or !onboarding.interactiveSetupDesired())
