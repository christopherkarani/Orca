...
pub fn command(io: std.Io, argv: []const []const u8, stdout: anytype, stderr: anytype) !u8 {
    ...
    const audit_session = commandAuditSession(io, session_id, workspace_root, "orca apply");
    ...
    const result = intercept.files.applyStaged(io, allocator, workspace_root, session_id, options.file, audit_context) catch |err| {
...
fn parseOptions(io: std.Io, argv: []const []const u8, stdout: anytype, stderr: anytype) !Options {
    ...
    while (index < argv.len) : (index += 1) {
        ...
        if (std.mem.eql(u8, arg, "--help") or std.mem.eql(u8, arg, "-h")) {
            _ = try help.writeCommand(io, stdout, "apply");
            ...
            try suggestions.writeUnknownOption(stderr, "orca apply", arg, &.{ "--session", "--file", "--help", "-h" }, "apply");
            return error.Usage;
        }
    }
    return options;
